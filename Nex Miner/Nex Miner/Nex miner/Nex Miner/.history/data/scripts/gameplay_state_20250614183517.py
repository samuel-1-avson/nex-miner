# Cavyn Source/data/scripts/game_states/gameplay_state.py
...
# Imports for the new states it will transition to
from .perk_selection_state import PerkSelectionState
...
def load_img(path):
    # This assumes you have a Game.get_path method
    # Since we don't have direct access, we build the path manually. This is less robust
    # but works for this specific structure. A better way would be passing the Game object to it.
...
class GameplayState(State):
    ...
    def update(self):
        ...
        self.update_tile_interactions() # For things like chests
        self.update_items()
    ...
    def update_falling_tiles(self):
        ...
        for i, tile in sorted(enumerate(self.tile_drops), reverse=True):
            ...
            if ('glass_cannon' in self.active_perks and r.colliderect(self.player.rect)):
                temp_shield = self.player_shielded; self.player_shielded = False
                self.handle_death()
                self.player_shielded = temp_shield
            ...
            if check_pos in self.tiles:
                ...
                # Motherlode transforms into a normal chest on landing, spawning coins immediately
                ...
                # Greed tiles give coins when another tile lands on them
                    ...
                    self.tiles[place_pos] = {'type': tile[2], 'data': {}} # The falling tile still places itself
                ...
                # Normal landing
                    ...
                    # Destroying a chest with a tile
                    if self.tiles.get(check_pos, {}).get('type') == 'chest':
                        self.tiles[check_pos]['type'] = 'tile' # Replace chest with normal tile
    ...
    def update_items(self):
        # We need a list of solid tile rects for items to collide with
        ...
        if not self.dead:
            ...
            if not collisions['bottom']:
                for side in ['left', 'right']:
                    ...
                    if side == 'left':
                        player_side_x = self.player.rect.left - 2
                    else: # right
                        player_side_x = self.player.rect.right + 2
    ...
    def update_sparks(self):
        for i, spark in sorted(enumerate(self.sparks), reverse=True):
            # This is complex physics/particle code. Let's make sure it's stable.
            # Spark format: [[x,y], [vx,vy], size, decay, color, physics_on, gravity_mod, grounded_flag]
            if len(spark) < 8: spark.append(False) # Add grounded flag if not present
            ...
            # Handle horizontal collision for physics sparks
            if spark[5]:
    ...
    def update_scrolling(self):
        if self.tiles:
            # Find the highest row that has a full line of tiles across the playable area
            try:
                ...
                for y in range(highest_base_row, 0, -1):
                    ...
                    if is_full_row:
                        # Set the target scroll height to move this full row off-screen
                        self.target_height = (y - self.game.WINDOW_TILE_SIZE[1] + 2) * self.game.TILE_SIZE
                        break
            except ValueError: # self.tiles might be empty
                pass

        if abs(self.height - self.target_height) > 0.01:
            self.height += (self.target_height - self.height) / 10 # Smoothed scrolling
        
        # When scrolling is complete, clean up tiles that are now far off-screen
        if abs(self.height - self.target_height) < 0.2:
             self.height = self.target_height
             # Find tiles that are now scrolled way off the top and remove them
    ...
    def use_item(self):
        ...
        if item == 'warp':
            ...
            self.screen_shake = 12; # THIS is the improved logic:
            self.player.jumps = self.player.jumps_max + 1
            self.player.attempt_jump()
            ...
            place_pos = (place_pos_x, place_pos_y)
    ...
    def render_placed_tiles(self, surf):
        for pos, data in self.tiles.items():
            ...
            if blit_pos[1] < -self.game.TILE_SIZE or blit_pos[1] > self.game.DISPLAY_SIZE[1]:
                continue
            ...
            if data['type'] in ['tile', 'placed_tile'] and (pos[0], pos[1]-1) not in self.tiles: 
                surf.blit(self.tile_top_img, blit_pos)
            
            if data['type'] == 'chest':
                surf.blit(self.chest_img, (blit_pos[0], blit_pos[1] - self.game.TILE_SIZE))
                if random.randint(1, 20) == 1: 
                    self.sparks.append([[blit_pos[0]+self.game.TILE_SIZE//2+random.uniform(-6,6), blit_pos[1]-self.game.TILE_SIZE//2+random.uniform(-4,4)], [0, random.uniform(-0.5, 0.25)], random.uniform(2,4), 0.023, (12,8,2), True, 0.002])
            
            elif data['type'] == 'opened_chest': 
                surf.blit(self.opened_chest_img, (blit_pos[0], blit_pos[1] - self.game.TILE_SIZE))

    def render_falling_tiles(self, surf):
        for tile in self.tile_drops:
            pos = (tile[0], tile[1] - self.height)
            if pos[1] < -self.game.TILE_SIZE or pos[1] > self.game.DISPLAY_SIZE[1]:
                continue
            ...
            if tile[2] in ['magnetic', 'sticky', 'fragile', 'bounce', 'spike']:
                img_to_blit = getattr(self, f"{tile[2]}_tile_img")
