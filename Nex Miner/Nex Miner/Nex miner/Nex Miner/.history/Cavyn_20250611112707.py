...
# -- NEW CONTENT: ASSETS --
turret_tile_img = load_img('data/images/turret_tile.png')
bullet_img = load_img('data/images/bullet.png')
...
# -- NEW CONTENT: SOUNDS --
if 'shoot' in sounds: sounds['shoot'].set_volume(0.4)
if 'shield_break' in sounds: sounds['shield_break'].set_volume(0.7)
if 'time_slow' in sounds: sounds['time_slow'].set_volume(0.7)
...
item_icons = {
    ...
    'clock': load_img('data/images/clock_icon.png'),
}
...
class Item(Entity):
    ...
    def update(self, tiles):
        self.time += 1
        self.velocity[1] = min(self.velocity[1] + 0.2, 3) * game_speed_multiplier
        self.velocity[0] = normalize(self.velocity[0], 0.05 * game_speed_multiplier)
...
class Player(Entity):
    ...
    def update(self, tiles):
        ...
        self.velocity[1] = min(self.velocity[1] + 0.3 * game_speed_multiplier, 4)
...
dead = False
tiles, tile_drops, bg_particles, sparks = {}, [], [], []
game_timer, height, target_height, coins, end_coin_count = 0, 0, 0, 0, 0
current_item, master_clock, last_place, item_used = None, 0, 0, False
combo_multiplier, combo_timer = 1.0, 0
COMBO_DURATION = 180
run_coins_banked, upgrade_selection, screen_shake, new_high_score = False, 0, 0, False
# -- NEW CONTENT: GAME STATE --
projectiles = []
...
def handle_death():
    ...
    if player.shielded:
        ...
        if 'shield_break' in sounds:
            sounds['shield_break'].play()
        screen_shake = 12
        for i in range(50):
            angle = random.random() * math.pi * 2
            speed = random.random() * 2
            sparks.append([player.center.copy(), [math.cos(angle) * speed, math.sin(angle) * speed], random.random() * 3 + 2, 0.06, (70, 150, 250), True, 0.1])
        return
...
while True:
    ...
    if combo_timer > 0:
        combo_timer -= 1 * game_speed_multiplier
    ...
    for i, p in sorted(enumerate(bg_particles), reverse=True):
        size = p[2]
        if p[-1] != (0, 0, 0): size = size * 5 + 4
        p[2] -= 0.01 * game_speed_multiplier
        p[0][1] -= p[3] * game_speed_multiplier
        if size < 1:
        ...
        if size < 0: bg_particles.pop(i)

    if master_clock > 180:
        game_timer += 1 * game_speed_multiplier
    ...
    for i, tile in sorted(enumerate(tile_drops), reverse=True):
        tile[1] += 1.4 * game_speed_multiplier
        ...
        if r.colliderect(player.rect): handle_death()
        ...
        if check_pos in tiles:
            ...
            # -- NEW CONTENT: TURRET --
            if tile[2] == 'turret':
                tiles[place_pos]['data']['timer'] = 180 # 3 second cooldown
            
            sounds['block_land'].play()
            
            if abs(player.pos[0] - place_pos[0] * TILE_SIZE) < 40: screen_shake = max(screen_shake, 4)
        ...
        if tile[2] == 'greed': img_to_blit = greed_tile_img
        elif tile[2] == 'turret': img_to_blit = turret_tile_img
        display.blit(img_to_blit, (tile[0], tile[1] + height))
    ...
    for tile_pos, tile_data in tiles.items():
        if tile_data['type'] == 'fragile':
            if 'timer' in tile_data['data']:
                tile_data['data']['timer'] -= 1 * game_speed_multiplier
        ...
        # -- NEW CONTENT: TURRET --
        if tile_data['type'] == 'turret':
            if 'timer' in tile_data['data']:
                tile_data['data']['timer'] -= 1 * game_speed_multiplier
                # Wind-up visual
                if tile_data['data']['timer'] < 40:
                     if random.randint(1, 4) == 1:
                        sparks.append([[tile_pos[0] * 16 + 8, tile_pos[1] * 16 + 8], [random.random() - 0.5, random.random() - 0.5], random.random() * 2 + 1, 0.1, (255, 100, 100), False, 0])
                if tile_data['data']['timer'] <= 0:
                    if 'shoot' in sounds: sounds['shoot'].play()
                    angle = math.atan2(player.center[1] - (tile_pos[1] * 16 + 8), player.center[0] - (tile_pos[0] * 16 + 8))
                    projectiles.append([[tile_pos[0] * 16 + 8, tile_pos[1] * 16 + 8], [math.cos(angle) * 3, math.sin(angle) * 3], 300])
                    tile_data['data']['timer'] = 180 + random.randint(0, 60)
    ...
    for tile_pos in tiles:
        ...
        elif tile_type == 'turret': img_to_blit = turret_tile_img
        ...
        if tile_type == 'chest':
            ...
            if random.randint(1, 20) == 1:
                sparks.append([[tile_pos[0]*16+2+12*random.random(), (tile_pos[1]-1)*16+4+8*random.random()], [0, random.random()*0.25-0.5], random.random()*4+2, 0.023, (12,8,2), True, 0.002])
            if chest_r.colliderect(player.rect):
                sounds['chest_open'].play()
                combo_multiplier += 1.0
                combo_timer = COMBO_DURATION
                for i in range(50):
                    angle, speed = random.random() * 2 - 1, random.random() - 2
                    sparks.append([[tile_pos[0]*16+8, (tile_pos[1]-1)*16+8], [angle, speed], random.random()*3+3, 0.01, (12,8,2), True, 0.05])
                tiles[tile_pos]['type'] = 'opened_chest' 
                player.jumps += 1
                player.attempt_jump()
                player.velocity[1] = -3.5
                item_luck_threshold = 3 + save_data['upgrades']['item_luck']
                item_pool = ['warp', 'cube', 'jump', 'bomb', 'shield', 'clock']
                if random.randint(1, 5) < item_luck_threshold:
                    items.append(Item(animation_manager, (tile_pos[0]*16+5, (tile_pos[1]-1)*16+5), (6, 6), random.choice(item_pool), velocity=[random.random()*5-2.5, random.random()*2-5]))
                else:
    ...
    edge_rects = [pygame.Rect(0,player.pos[1]-300,16,600), pygame.Rect(16*(WINDOW_TILE_SIZE[0]-1),player.pos[1]-300,16,600)]

    for i, item in sorted(enumerate(items), reverse=True):
        lookup_pos = (int(item.center[0] // 16), int(item.center[1] // 16))
        if lookup_pos in tiles:
            items.pop(i)
            continue
    ...
    for i, p in sorted(enumerate(projectiles), reverse=True):
        p[0][0] += p[1][0] * game_speed_multiplier
        p[0][1] += p[1][1] * game_speed_multiplier
        p[2] -= 1 * game_speed_multiplier
        display.blit(bullet_img, (p[0][0], p[0][1] + height))
        if player.rect.collidepoint(p[0]):
            projectiles.pop(i)
            handle_death()
        elif p[2] <= 0:
            projectiles.pop(i)
    ...
    if not dead:
        ...
        if collisions['bottom']:
            tile_pos_below = (player.rect.midbottom[0]//16, player.rect.midbottom[1]//16)
            if tile_pos_below in tiles:
                if tiles[tile_pos_below]['type'] == 'fragile' and 'timer' not in tiles[tile_pos_below]['data']:
                    sounds['block_land'].play()
                    tiles[tile_pos_below]['data']['timer'] = 90
                elif tiles[tile_pos_below]['type'] == 'bounce':
                    player.velocity[1], player.jumps, screen_shake = -9, player.jumps_max, 10
                    sounds['super_jump'].play()
                    combo_multiplier += 0.5; combo_timer = COMBO_DURATION
                elif tiles[tile_pos_below]['type'] == 'spike': 
                    handle_death()
                    player.velocity = [0, -4]

    else:
        player.opacity, player.rotation = 80, player.rotation - 16
        player.update([])
    
    player.render(display, (0, -int(height)))
    # -- NEW CONTENT: SHIELD --
    if player.shielded:
        shield_size = 12 + math.sin(master_clock / 15) * 2
        shield_surf = pygame.Surface((shield_size*2, shield_size*2))
        pygame.draw.circle(shield_surf, (70, 150, 250), (shield_size, shield_size), shield_size)
        shield_surf.set_colorkey((0,0,0))
        shield_surf.set_alpha(80)
        display.blit(shield_surf, (player.center[0] - shield_size, player.center[1] + height - shield_size))


    for i, spark in sorted(enumerate(sparks), reverse=True):
        if len(spark) < 8: spark.append(False)
        if not spark[-1]: spark[1][1] = min(spark[1][1] + spark[-2], 3) * game_speed_multiplier
        spark[0][0] += spark[1][0] * game_speed_multiplier
        if spark[5]:
            if ((int(spark[0][0]//16), int(spark[0][1]//16)) in tiles) or (spark[0][0]<16) or (spark[0][0]>DISPLAY_SIZE[0]-16):
                spark[0][0] -= spark[1][0] * game_speed_multiplier
                spark[1][0] *= -0.7
        spark[0][1] += spark[1][1] * game_speed_multiplier
        if spark[5]:
            if (int(spark[0][0]//16), int(spark[0][1]//16)) in tiles:
                spark[0][1] -= spark[1][1] * game_speed_multiplier
                spark[1][1] *= -0.7
                if abs(spark[1][1]) < 0.1:
                    spark[1][1], spark[-1] = 0, True
        spark[2] -= spark[3] * game_speed_multiplier
        if spark[2] <= 1: sparks.pop(i)
        else:
            glow_size = int(spark[2])
            c = spark[4]
            display.blit(glow_img(glow_size * 1.5 + 2, (int(c[0]/2), int(c[1]/2), int(c[2]/2))), (spark[0][0] - glow_size*2, spark[0][1]+height-glow_size*2), special_flags=BLEND_RGBA_ADD)
            display.blit(glow_img(glow_size, c), (spark[0][0]-glow_size, spark[0][1]+height-glow_size), special_flags=BLEND_RGBA_ADD)
            display.set_at((int(spark[0][0]), int(spark[0][1] + height)), (255, 255, 255))
    
    # -- NEW CONTENT: TIME-SLOW --
    if slow_time_timer > 0:
        vignette = pygame.Surface(DISPLAY_SIZE)
        vignette.set_alpha(int(100 * (slow_time_timer / 300)))
        vignette.fill((20, 50, 80))
        display.blit(vignette, (0,0), special_flags=BLEND_RGBA_MULT)
    ...
    if not dead:
        hud_panel = pygame.Surface((DISPLAY_SIZE[0], 24)); hud_panel.set_alpha(100); hud_panel.fill((0, 0, 1)); display.blit(hud_panel, (0, 0))
        ...
        if combo_multiplier > 1.0:
            ...
            black_font.render(combo_text, display, (DISPLAY_SIZE[0]//2 - text_width//2+1, 6)); white_font.render(combo_text, display, (DISPLAY_SIZE[0]//2-text_width//2, 5))
            bar_width = (combo_timer/COMBO_DURATION)*40; bar_x = DISPLAY_SIZE[0]//2-20
            pygame.draw.rect(display, (0,0,1), [bar_x+1, 15, 40, 3]); pygame.draw.rect(display, (251,245,239), [bar_x, 14, bar_width, 3])
        ...
        y_offset = 30
        black_font.render('game over', display, (DISPLAY_SIZE[0]//2 - white_font.width('game over')//2+1, y_offset+1)); white_font.render('game over', display, (DISPLAY_SIZE[0]//2 - white_font.width('game over')//2, y_offset)); y_offset += 15
        hs_text = f"High Score: {save_data['high_score']}"; hs_font = white_font
        if new_high_score and master_clock%40<20: hs_font = Font('data/fonts/small_font.png', (255,230,90))
        ...
        if end_coin_count != coins:
            ...
            if not run_coins_banked: save_data['banked_coins']+=end_coin_count; write_save(save_data); run_coins_banked = True
        ...
        if run_coins_banked:
            ...
            for i, key in enumerate(upgrade_keys):
                upgrade, level = UPGRADES[key], save_data['upgrades'][key]; cost = int(upgrade['base_cost'] * (1.5**level))
                color = (255,230,90) if i == upgrade_selection else white_font.color
                temp_font = Font('data/fonts/small_font.png', color); temp_black_font = Font('data/fonts/small_font.png', (1,1,1))
                level_text = f"[{level}/{upgrade['max_level']}]" if level<upgrade['max_level'] else "[MAX]"; name_text = f"{upgrade['name']} {level_text}"
                temp_black_font.render(name_text, display, (11, y_offset+1)); temp_font.render(name_text, display, (10, y_offset))
                if level < upgrade['max_level']:
                    cost_text = str(cost); display.blit(coin_icon, (DISPLAY_SIZE[0]-14-white_font.width(cost_text), y_offset-2))
                    temp_black_font.render(cost_text, display, (DISPLAY_SIZE[0]-white_font.width(cost_text)-9, y_offset+1)); temp_font.render(cost_text, display, (DISPLAY_SIZE[0]-white_font.width(cost_text)-10, y_offset))
                y_offset += 15
            y_offset += 5
            help_text = "Up/Down:Select - Enter:Buy"; black_font.render(help_text, display, (DISPLAY_SIZE[0]//2-white_font.width(help_text)//2+1, y_offset+1)); white_font.render(help_text, display, (DISPLAY_SIZE[0]//2-white_font.width(help_text)//2, y_offset)); y_offset += 15
            black_font.render('press R to restart', display, (DISPLAY_SIZE[0]//2-white_font.width('press R to restart')//2+1, y_offset+1)); white_font.render('press R to restart', display, (DISPLAY_SIZE[0]//2-white_font.width('press R to restart')//2, y_offset))
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            if not dead:
                ...
                if event.key in [K_e,K_x] and current_item:
                    ...
                    if current_item == 'warp':
                        sounds['warp'].play(); screen_shake = 15; max_point = min(enumerate(stack_heights),key=lambda x:x[1])
                        player.pos = [(max_point[0]+1)*16+4, (max_point[1]-1)*16]
                    ...
                    if current_item == 'cube':
                        sounds['block_land'].play(); place_pos = (int(player.center[0]//16), int(player.pos[1]//16)+1)
                        stack_heights[place_pos[0]-1] = place_pos[1]; current_base_row = max(tiles,key=lambda x:x[1])[1]-1 if tiles else WINDOW_TILE_SIZE[1]-2
                        for i in range(place_pos[1], current_base_row+2): tiles[(place_pos[0],i)] = {'type':'placed_tile','data':{}}
                    if current_item == 'bomb':
                        if 'explosion' in sounds: sounds['explosion'].play(); screen_shake = 25
                        tiles_to_bomb = [list(tp) for tp in tiles if math.sqrt((player.center[0]-(tp[0]*16+8))**2 + (player.center[1]-(tp[1]*16+8))**2) < 4*16]
                        for tile_pos in tiles_to_bomb:
                            if tuple(tile_pos) in tiles:
                                for i in range(15): sparks.append([[tile_pos[0]*16+8, tile_pos[1]*16+8], [math.cos(i/15*math.pi*2)*2, math.sin(i/15*math.pi*2)*2], random.random()*4+2, 0.08, (12,2,2), True, 0.1])
                                del tiles[tuple(tile_pos)]
                        recalculate_stack_heights()
                    if current_item == 'clock':
                        if 'time_slow' in sounds: sounds['time_slow'].play()
                        slow_time_timer = 300 # 5 seconds
                    current_item = None
            else:
                if run_coins_banked:
                    ...
                    if event.key == K_RETURN:
                        key, upgrade, level = upgrade_keys[upgrade_selection], UPGRADES[upgrade_keys[upgrade_selection]], save_data['upgrades'][upgrade_keys[upgrade_selection]]
            ...
            if event.key == K_r and dead:
                player = Player(animation_manager, (DISPLAY_SIZE[0]//2-5, -20), (8,16), 'player')
                player.jumps_max += save_data['upgrades']['jumps']; player.speed += save_data['upgrades']['speed']*0.08; player.jumps = player.jumps_max
                dead, tiles, tile_drops, sparks = False, {}, [], []
                game_timer, height, target_height, coins, end_coin_count = 0,0,0,0,0
                current_item, master_clock, last_place, item_used = None,0,0,False
                combo_multiplier, combo_timer, run_coins_banked, screen_shake = 1.0, 0, False, 0
                new_high_score, current_palette_index = False, 0
                ...
                stack_heights = [WINDOW_TILE_SIZE[1]-1 for i in range(WINDOW_TILE_SIZE[0]-2)]; stack_heights[0]-=1; stack_heights[-1]-=1; items=[]
        if event.type == KEYUP:
