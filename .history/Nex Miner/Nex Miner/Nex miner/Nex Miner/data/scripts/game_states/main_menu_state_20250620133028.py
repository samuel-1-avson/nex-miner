...
class MainMenuState(State):
    ...
    def enter_state(self):
        ...
        self.selector_y = self.selectorfrom ..core_funcs import load_img
from ..gameplay_state import GameplayState, render_panel_9slice
from .upgrade_shop_state import UpgradeShopState
from .settings_state import SettingsState
from .character_select_state import CharacterSelectState
...
class MainMenuState(State):
    def __init__(self, game):
        ...
        for y in range(0, self.game.DISPLAY_SIZE[1], 2):
            pygame.draw.line(self.scanline_surface, (0, 10, 15, 80), (0, y), (self.game.DISPLAY_SIZE[0], y), 1)
        try:
            self.logo_img = pygame.image.load(self.game.get_path('data', 'images', 'Nex Miner_Logo.png')).convert_alpha()
        except pygame.error:
            self.logo_img = None # Fallback if logo is missing
    ...
    def render_title(self, surface):
        # Define starting Y position for the title area
        y_pos = 15

        if self.logo_img:
            # Add a subtle sine wave bob for a "floating" effect
            y_bob = math.sin(self.master_clock / 40) * 2
            logo_pos_x = surface.get_width() // 2 - self.logo_img.get_width() // 2
            # Blit the logo at the current y_pos
            surface.blit(self.logo_img, (logo_pos_x, y_pos + y_bob))
            # Advance y_pos to be below the logo for the next element
            y_pos += self.logo_img.get_height() + 5
        else: # Fallback to text if logo fails to load
            # Render the glitchy text at the current y_pos
            self.render_glitchy_text(surface, "NEX MINER", (surface.get_width()//2, y_pos), self.font_main, scale=3, center=True)
            # Advance y_pos to be below the text title for the next element (font height of 8 * scale of 3 = 24)
            y_pos += 24 + 5
        
        # Now, render the sub-header at the new, correctly calculated y_pos
        sub_header = "//:HUB INTERFACE v3.1"
        w_sub = self.font_main.width(sub_header)
        self.font_main.render(sub_header, surface, (surface.get_width()//2 - w_sub//2, y_pos))
