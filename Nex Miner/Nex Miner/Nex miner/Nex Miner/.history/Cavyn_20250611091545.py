...
def upgrade_loop():
    ...
    display.blit(coin_icon, (DISPLAY_SIZE[0] // 2 - (white_font.width(bank_text) + 9) // 2, 34))
    white_font.render(bank_text, display, (DISPLAY_SIZE[0] // 2 - (white_font.width(bank_text) - 9) // 2 + 5, 35))
    ...
    for i, key in enumerate(upgrade_keys):
        ...
        h_color = (251, 245, 239)
        nh_color = (100, 100, 120)
        cant_afford_color = (150, 40, 40)
        
        name_font = Font('data/fonts/small_font.png', h_color if i == upgrade_selection else nh_color)
        
        name_font.render(f"{upgrade_info['name']} [Lvl {current_level}]", display, (20, y_offset))
        ...
        cost_font.render(cost_text, display, (DISPLAY_SIZE[0] - white_font.width(cost_text) - 20, y_offset))
    ...
    white_font.render("Press Enter/Space to buy.", display, (20, DISPLAY_SIZE[1] - 30))
    white_font.render("Press ESC to go back.", display, (20, DISPLAY_SIZE[1] - 20))
                    ...
                    if event.key in [K_e, K_x]:
                        ...
                        if current_item == 'warp':
                            ...
                            if stack_heights:
                                ...
                                for i in range(60):
                                    angle = random.random() * math.pi * 2; speed = random.random() * 3; physics_on = random.choice([False, True])
                                ...
                                for i in range(60):
                                    angle = random.random() * math.pi * 2; speed = random.random() * 1.75; physics_on = random.choice([False, True, True])
                                    sparks.append([player.center.copy(), [math.cos(angle) * speed, math.sin(angle) * speed], random.random() * 3 + 3, 0.02, (12, 8, 2), physics_on, 0.1 * physics_on])
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                if event.key in [K_DOWN, K_s]:
                    upgrade_selection = (upgrade_selection + 1) % len(UPGRADE_DEFINITIONS)
