...
class GameplayState(State):
    ...
    def load_state_assets(self):
        ...
        self.item_icons = {
            'cube': load_img(self.game.get_path('data', 'images', 'cube_icon.png')), 'warp': load_img(self.game.get_path('data', 'images', 'warp_icon.png')),
            'jump': load_img(self.game.get_path('data', 'images', 'jump_icon.png')), 'bomb': load_img(self.game.get_path('data', 'images', 'bomb_icon.png')),
            'freeze': load_img(self.game.get_path('data', 'images', 'freeze_icon.png')), 'shield': load_img(self.game.get_path('data', 'images', 'shield_icon.png')),
            'hourglass': load_img(self.game.get_path('data', 'images', 'hourglass_icon.png')), # <-- ADDED
        }
    ...
    def enter_state(self):
        super().enter_state()
        if self.perks_gained_this_run > 0 and self.perks_gained_this_run % 2 == 0:
    ...
    def reset(self):
        ...
        self.screen_shake, self.player_shielded = 0, False
        self.combo_shield_used = False # <-- ADDED
    ...
    # The rest of GameplayState is unchanged and omitted for brevity.
    # The previously modified methods like update_player, update_falling_tiles, etc.
    # are already correct from the last step.

    # --- MAIN LOOPS ---
    def handle_events(self, events):
        # ... (rest of file from here is unchanged)
    ...
    def handle_death(self):
        ...
        if self.dead: return
        self.dead = True # Prevent multiple triggers
        ...
        # Create a snapshot of the screen for the game over background
        background_surf = self.game.display.copy()
        self.render(background_surf)
        ...
        if self.combo_timer > 0:
            ...
            if self.combo_timer <= 0 and self.combo_multiplier > 1.0:
                ...
                if shield_level > 0 and not self.combo_shield_used:
                    ...
                    # Gold sparks for a special event
                    for i in range(30):
                        angle, speed = random.random() * math.pi * 2, random.random() * 2
    ...
    def update_tile_spawning(self):
        ...
        if self.game_timer > 10 + 25 * (20000 - min(20000, self.master_clock)) / 20000:
            ...
            # --- START: New Simplified & Robust Logic ---

            # Step 1: Find all columns that are not at the very top of the play area.
            # A tile can't spawn if its column is already full.
            # We check a few tiles down from the top to give some headroom.
            valid_columns = []
            for i in range(len(self.stack_heights)):
                if self.stack_heights[i] > 4: # Only consider columns with at least 4 tiles of space
                    valid_columns.append(i)

            # If for some reason all columns are full, do nothing to prevent a crash.
            if not valid_columns:
                return

            # Step 2: Choose a column.
            chosen_col_index = -1
            # If there are multiple choices, avoid the last placed column.
            if len(valid_columns) > 1 and self.last_place in valid_columns:
                choices = [col for col in valid_columns if col != self.last_place]
                chosen_col_index = random.choice(choices)
            # If there's only one choice, we MUST use it, even if it's a repeat.
            elif len(valid_columns) > 0:
                chosen_col_index = random.choice(valid_columns)
            # If we still haven't found a column (should be impossible but is a good failsafe), exit.
            else:
                return

            self.last_place = chosen_col_index

            # Step 3: Determine tile type (this part was mostly fine).
            biome = self.game.biomes[self.current_biome_index]
            final_tile_type = 'tile' # Default
            
            elite_roll = random.randint(1, 100)
            if elite_roll <= 2:
                final_tile_type = 'motherlode'
            elif elite_roll <= 5:
                final_tile_type = 'unstable'
            else:
            ...
            # --- END: New Simplified & Robust Logic ---               
    def update_falling_tiles(self):
        ...
        for i, tile in sorted(enumerate(self.tile_drops), reverse=True):
            ...
            if self.player.dash_timer > 0 and r.colliderect(self.player.rect):
                ...
                # MODIFIED: Check for 'Brittle Blocks' curse
    ...
    def update_tile_interactions(self):
        """Handle interactions like opening chests."""
        if self.dead: return
            ...
            if data['type'] == 'chest':
                ...
                if r.colliderect(self.player.rect):
                    ...
                    if random.randint(1, 5) < item_luck:
                        # --- MODIFIED: Add Hourglass to loot table
    ...
    def update_player(self):
        if not self.dead:
            ...
            # --- MODIFIED: Temporarily apply 'Heavy Feet' curse effect ---
            original_speed = self.player.speed
            if 'heavy_feet' in self.active_curses:
                self.player.speed *= 0.85
            
            collisions = self.player.update(self.tile_drop_rects + edge_rects + tile_rects, self.time_scale)

            # IMPORTANT: Reset speed so the effect doesn't compound
            self.player.speed = original_speed
            # --- END MODIFICATION ---

            if not collisions['bottom']:
                for side in ['left', 'right']:
                    player_side_y = self.player.rect.centery
                    player_side_x = self.player.rect.left - 2 if side == 'left' else self.player.rect.right + 2
                    tile_pos_side = (player_side_x // self.game.TILE_SIZE, player_side_y // self.game.TILE_SIZE)
                    if tile_pos_side in self.tiles and self.tiles.get(tile_pos_side, {}).get('type') == 'sticky':
    ...
    def use_item(self):
        ...
        # --- NEW ITEM LOGIC ---
            ...
            if self.combo_multiplier > 1.0:
                self.combo_timer = self.game.COMBO_DURATION # Refill the timer
                for i in range(40):
                    angle, speed = random.random() * math.pi * 2, random.random() * 1.5
    ...
    def render_placed_tiles(self, surf):
        for pos, data in self.tiles.items():
            ...
            img_to_blit = img_map.get(data['type'], self.tile_img)
            ...
            if data['type'] == 'chest':
                ...
                if random.randint(1, 20) == 1: self.sparks.append([[blit_pos[0]+self.game.TILE_SIZE//2+random.uniform(-6,6), blit_pos[1]-self.game.TILE_SIZE//2+random.uniform(-4,4)], [0, random.uniform(-0.5, 0.25)], random.uniform(2,4), 0.023, (12,8,2), True, 0.002])
            elif data['type'] == 'opened_chest': surf.blit(self.opened_chest_img, (blit_pos[0], blit_pos[1] - self.game.TILE_SIZE))
    ...
    def render_sparks(self, surf):
        for spark in self.sparks:
            ...
            if size > 0:
                surf.blit(glow_img(int(size*1.5+2), (int(spark[4][0]/2),int(spark[4][1]/2),int(spark[4][2]/2))), (pos[0]-size*2, pos[1]-size*2), special_flags=BLEND_RGBA_ADD)
