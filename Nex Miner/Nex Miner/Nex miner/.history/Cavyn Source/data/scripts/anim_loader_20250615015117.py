...
class AnimationData:
    def __init__(self, path, colorkey=None):
        self.id = path.split(os.sep)[-1] # Use os.sep for OS compatibility
        self.image_list = []
        ...
        for img_filename in os.listdir(path):
            if img_filename.endswith('.png'):
                ...
                frame_number = int(img_filename.split('.')[0].split('_')[-1])
...
class Animation:
    ...
    def calc_img(self):
        if not self.data.frame_surfs:
            self.img = pygame.Surface((1,1)) # Fallback if no frames loaded
            return
...
class AnimationManager:
    ...
    def new(self, anim_id):
        if anim_id in self.animations:
            ...
            return Animation(AnimationData('.', colorkey=COLORKEY))