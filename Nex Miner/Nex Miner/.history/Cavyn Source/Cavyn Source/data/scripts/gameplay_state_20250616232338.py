...
class GameplayState(State):
    ...
    def handle_death(self):
        ...
        if self.player_shielded:
            ...
            if 'safeguard' in self.active_perks:
                # Add a brief invincibility timer or effect
                pass
            for i in range(60):
    ...
    def update_biome(self):
        ...
        if next_biome_index != self.current_biome_index:
            ...
            # Update background for Sunken City
            if self.game.biomes[self.current_biome_index]['name'] == 'The Sunken City':
                 self.game.backgrounds['far'] = pygame.transform.scale(load_img(self.game.get_path('data','images','background_sunken.png')), self.game.DISPLAY_SIZE)
                 self.game.backgrounds['near'] = None
            else:
                self.game.load_biome_bgs(self.game.biomes[self.current_biome_index])
    ...
    def render_borders(self, surf):
        ...
        # --- NEW: Border color change for Hardcore ---
        if self.mode == "hardcore":
            self.border_img.set_alpha(200)
            self.border_img.fill((255,50,50, 200), special_flags=pygame.BLEND_RGBA_MULT)
