...
class GameplayState(State):
    ...
    def update_tile_spawning(self):
        ...
        if self.game_timer > 10 + 25 * (20000 - min(20000, self.master_clock)) / 20000:
            ...
            # --- MODIFIED: Bug Fix and cleaner logic ---
            ...
            if elite <= 2: tile_type = 'motherlode'
                ...
                elif roll < 8: tile_type = 'greed' # Normal Greed Tile spawn
                elif roll == 8: tile_type = 'spike'
                
                # Check biome specific spawns
                for special, chance in biome.get('special_spawn_rate', {}).items():
                    if roll == chance: tile_type = special

            # Apply the "High Stakes" curse AFTER determining the tile type
            if tile_type == 'greed' and 'high_stakes' in self.active_curses:
                tile_type = 'spike'
