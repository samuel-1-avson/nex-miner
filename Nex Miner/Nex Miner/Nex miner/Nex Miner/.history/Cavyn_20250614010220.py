...
class Player(Entity):
    ...
    def update(self, tiles):
        ...
        # Apply scaling based on jumping and landing squash effect
        target_scale_y = 0.7 if self.jumping else 1.0
        
        if self.squash_timer > 0:
            # A sine curve from 1 down to 0 for a smooth squash recovery
            squash_magnitude = math.sin((self.squash_timer / self.SQUASH_DURATION) * (math.pi / 2)) * 0.4
            self.scale = [1.0 + squash_magnitude, 1.0 - squash_magnitude]
        else:
            self.scale = [1.0, target_scale_y]
        ...
        if collisions['bottom']:
            if self.air_time > 1: # Just landed
