...
from .entities.turret import Turret # <<< NEW
from .entities.turret_projectile import TurretProjectile # <<< NEW
...
class GameplayState(State):
    ...
    def reset(self):
        ...
        # --- NEW: Add lists for turrets ---
        self.turrets = []
        self.turret_projectiles = []

        self.ghosts = []
        # ... (the rest of the reset method is the same)
    ...
    def update(self):
        ...
        # --- NEW: Call update methods for turrets ---
        self.update_turrets()
    ...
    def update_turrets(self):
        ...
        for i, turret in sorted(enumerate(self.turrets), reverse=True):
            ...
            for j, p_proj in sorted(enumerate(self.projectiles), reverse=True):
                if p_proj.rect.colliderect(turret.rect):
            ...
            if self.player.dash_timer > 0 and self.player.rect.colliderect(turret.rect):
    ...
    def update_turret_projectiles(self):
        for i, proj in sorted(enumerate(self.turret_projectiles), reverse=True):
            # The projectile's update returns True when it should be deleted
    ...
    def destroy_turret(self, index, turret, cause):
        # Safely remove turret from the list
        ...
        for _ in range(30):
            ...
            self.sparks.append([turret.center, [math.cos(angle) * speed, math.sin(angle) * speed], self.random.random() * 3 + 2, 0.08, (180, 50, 50), True, 0.1])
        
        # Give rewards if destroyed by player action
    ...
    def update_falling_tiles(self):
        ...
        for i, tile in sorted(enumerate(self.tile_drops), reverse=True):
            ...
            if check_pos_real in self.tiles:
                ...
                # --- NEW: TURRET SPAWN LOGIC ---
                if tile[2] in ['tile', 'placed_tile'] and self.mode != "zen":
                    # 10% chance to spawn a turret on a regular tile
                    if self.random.randint(1, 100) <= 10:
                        turret_pos = [place_pos[0] * self.game.TILE_SIZE + 4, (place_pos[1] - 1) * self.game.TILE_SIZE]
                        self.turrets.append(Turret(self.game.animation_manager, turret_pos, (8, 16), 'turret', self, place_pos))
    ...
    def bomb_item(self): # Modified bomb to destroy turrets
        if 'explosion' in self.game.sounds: self.game.sounds['explosion'].play()
        self.screen_shake = 25
        bomb_radius_sq = (4 * self.game.TILE_SIZE) ** 2

        # Bomb tiles
        to_bomb = [list(tp) for tp in self.tiles if (self.player.center[0] - (tp[0] * 16 + 8))**2 + (self.player.center[1] - (tp[1] * 16 + 8))**2 < bomb_radius_sq]
        for pos in to_bomb:
        ...
        # --- NEW: Bomb Turrets ---
        for i, turret in sorted(enumerate(self.turrets), reverse=True):
            if (self.player.center[0] - turret.center[0])**2 + (self.player.center[1] - turret.center[1])**2 < bomb_radius_sq:
    ...
    def render(self, surface):
        ...
        # --- NEW: Render Turrets and their projectiles ---
    ...
    # All other methods like `check_challenge_conditions`, `handle_death`, `update_ghosts`, etc.,
    # remain UNCHANGED from the last version you have.
    # No need to paste them all again. Only `bomb_item`, `update`, `reset`, `update_falling_tiles`, and `render`
    # needed direct changes or calls to new methods.
    # Replace `bomb_item` call in `use_item` with `self.bomb_item()` if you have it. The logic is now inside this class.
    # I've put the bomb logic in a separate function to make `use_item` cleaner. Update the 'bomb' case in use_item:
    def use_item(self):
        # ... other cases ...
        elif item == 'bomb':
            self.bomb_item()
...
# --- Final Check on all other unmodified methods ---
    ...
    def update_ambient_hazards(self):
        if 'ambient_hazard' in self.game.biomes[self.current_biome_index]:
            ...
            if self.player.pos[1] > self.plasma_y:
                self.handle_death()
