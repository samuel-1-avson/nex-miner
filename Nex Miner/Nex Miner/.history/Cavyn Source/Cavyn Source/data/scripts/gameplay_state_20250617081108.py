...
class GameplayState(State):
    ...
    def reset(self):
        ...
        self.curse_rerolls_left = self.game.save_data['upgrades'].get('curse_reroll', 0) # NEW
    ...
    def update_biome(self):
        ...
        if next_biome_index != self.current_biome_index:
            ...
            self.combo_shield_used = False # <-- ADD THIS LINE to reset the shield
        ...
        for i, crystal_rect in sorted(enumerate(self.crystals), reverse=True):
            ...
            if self.player.dash_timer > 0 and self.player.rect.colliderect(crystal_render_rect):
                ...
                for k in range(20):
                    ...
                    self.sparks.append([list(crystal_render_rect.center), [math.cos(angle) * speed, math.sin(angle) * speed], random.uniform(2, 5), 0.08, (200, 220, 255), True, 0.1])
                continue
    ...
    def update_falling_tiles(self):
        ...
        for i, tile in sorted(enumerate(self.tile_drops), reverse=True):
            ...
            if self.player.dash_timer > 0 and r.colliderect(self.player.rect):
                ...
                for k in range(15):
                    angle, speed = random.random() * math.pi * 2, random.random() * 2.5
                    self.sparks.append([list(r.center), [math.cos(angle) * speed, math.sin(angle) * speed], random.uniform(2, 5), 0.08, (251, 245, 239), True, 0.1])
            ...
            if check_pos_real in self.tiles:
                ...
                # Logic for different tile landing behaviors...
                ...
                if random.random() < self.tile_coin_drop_chance:
                     self.items.append(Item(self.game.animation_manager, (place_pos[0] * 16 + 5, place_pos[1] * 16 + 5), (6, 6), 'coin', self, velocity=[random.random() * 2 - 1, random.random() * -2]))
    ...
    def update_placed_tiles(self):
        ...
        for tile_pos, tile_data in self.tiles.items():
            ...
            if tile_data['type'] == 'unstable' and 'timer' in tile_data['data']:
                ...
                if random.randint(1, 8) == 1: self.sparks.append([[(tile_pos[0] + 0.5) * self.game.TILE_SIZE, (tile_pos[1] + 0.5) * self.game.TILE_SIZE - self.height], [random.uniform(-0.5, 0.5), random.uniform(-0.5, 0.5)], random.uniform(2, 4), 0.1, (255, 100, 20), False, 0])
        ...
        if to_remove:
            for p in to_remove:
                ...
                if random.random() < self.tile_coin_drop_chance:
                     self.items.append(Item(self.game.animation_manager, (p[0] * 16 + 5, p[1] * 16 + 5), (6, 6), 'coin', self, velocity=[random.random() * 2 - 1, random.random() * -2]))
            self.recalculate_stack_heights()
            self.game.sounds['explosion'].play()
            self.screen_shake = max(self.screen_shake, 12)


    # The remaining methods (`update_tile_interactions`, `update_items`, `update_projectiles`, `update_player`, 
    # `update_sparks`, `update_scrolling`, `fire_projectile`, `use_item`, and all render methods)
    # are very long and mostly unchanged from the previous submission, so they are omitted here.
    # The crucial fix has been applied at the top of the file.
                ...
                if tile_pos_below in self.tiles:
                    ...
                    if tile_type == 'fragile' and 'timer' not in self.tiles[tile_pos_below]['data']: self.game.sounds['block_land'].play(); self.tiles[tile_pos_below]['data']['timer'] = 90
                        ...
                        for _ in range(4):
                            ...
                            self.sparks.append([list(self.player.rect.midbottom), [math.cos(angle) * speed, math.sin(angle) * speed - 0.5], random.random() * 2 + 1, 0.1, (100, 150, 255), True, 0.02])
    ...
    def use_item(self):
        if 'butter_fingers' in self.active_curses and random.random() < 0.5:
    ...
    def render_placed_tiles(self, surf):
        for pos, data in self.tiles.items():
            ...
            if data.get('type') == 'conduit' and self.master_clock % 4 == 0:
                self.sparks.append([
                    [blit_pos[0] + random.random() * 16, blit_pos[1] + random.random() * 16],
                    [0, 0], random.random() * 1.5, 0.1, (150, 180, 255), False, 0
                ])
