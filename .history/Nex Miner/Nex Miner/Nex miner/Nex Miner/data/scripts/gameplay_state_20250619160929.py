...
class GameplayState(State):
    ...
    def render_sparks(self, surf):
        for spark in self.sparks:
            ...
            # --- FIX: Robustly center the glow surfaces ---

            # Outer glow (larger, darker)
            glow_size = int(size * 1.5 + 2)
            glow_color = (int(spark[4][0] / 2), int(spark[4][1] / 2), int(spark[4][2] / 2))
            outer_glow_surf = glow_img(glow_size, glow_color)
            outer_pos = (pos[0] - outer_glow_surf.get_width() // 2, pos[1] - outer_glow_surf.get_height() // 2)
            surf.blit(outer_glow_surf, outer_pos, special_flags=pygame.BLEND_RGBA_ADD)

            # Inner particle (smaller, brighter)
            inner_glow_surf = glow_img(size, spark[4])
            inner_pos = (pos[0] - inner_glow_surf.get_width() // 2, pos[1] - inner_glow_surf.get_height() // 2)
            surf.blit(inner_glow_surf, inner_pos, special_flags=pygame.BLEND_RGBA_ADD)
