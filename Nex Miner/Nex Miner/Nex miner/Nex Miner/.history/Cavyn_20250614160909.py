...
BIOMES = [
    {
        ...
        # <-- MODIFIED: Added new elite tiles to the available list -->
...
unstable_tile_img = load_img('data/images/unstable_tile.png') # <-- NEW
    ...
    if not perk_selection_active:
        ...
        if master_clock > 180:
            ...
            if game_timer > 10 + 25 * (20000 - min(20000, master_clock)) / 20000:
                ...
                if elite_roll <= 2: # 2% chance
                    tile_type = 'motherlode'
                elif elite_roll <= 5: # 3% chance (rolls 3, 4, 5)
                    tile_type = 'unstable'
                else:
                    # If no elite, roll for a normal tile
        ...
        for i, tile in sorted(enumerate(tile_drops), reverse=True):
            if freeze_timer <= 0: # Check if time is frozen
                ...
                if tile[2] == 'motherlode':
                    fall_speed = 1.8 # Make it fall faster!
            ...
            if check_pos in tiles:
                ...
                # <-- MODIFIED: Handle unstable tile creation -->
                tile_data_dict = {'type': tile[2], 'data': {}}
                if tile[2] == 'unstable':
                    tile_data_dict['data']['timer'] = 180 # 3 seconds @ 60 FPS
                    ...
                    if tile_data['data']['timer'] <= 0:
                        ...
                        for i_inner in range(40):
                            ...
                            sparks.append([
                                ...
                                random.random() * 5 + 3, 0.08, (255, 120, 40), True, 0.1
                            ])
        ...
        for tile_pos in tiles_to_remove:
            if tile_pos in tiles:
                del tiles[tile_pos]
        # Recalculate only if tiles were actually removed
        if tiles_to_remove:
            recalculate_stack_heights()
        ...
        if not dead:
            ...
            ### FIXED: This is now the single, consolidated section for drawing the right-side HUD elements ###
            ...
            # --- FOCUS DASH METER ---
            focus_bar_x = DISPLAY_SIZE[0] - 25 # X position for the bar
