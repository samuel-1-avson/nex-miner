...
class PlayerHubState(State):
                ...
                if self.tabs[self.current_tab] == "ARTIFACTS":
                    ...
                    if unlocked_artifacts:
                        if event.key in [pygame.K_DOWN, pygame.K_s]:
                            ...
                            self.game.sounds['artifact_equip'].play()
    ...
    def render_oracle(self, surface, p_rect, y):
        ...
        if not self.game.ai_agent:
            ...
            prompt_text = "Ask a question. Try 'What is the best way to earn coins?' or 'Tell me about the Ninja character.'"
            self.game.white_font.render(prompt_text, surface, (response_box.x, response_box.y), line_width=response_box.width)
        
        if self.game.save_data.get('active_prophecy'):
             prophecy_prompt = "[PROPHECY RECORDED]"
        elif self.game.save_data['upgrades'].get('prophecy_clarity', 0) > 0:
            prophecy_prompt = "[P] Seek Clarified Prophecy"
        else:
        ...
        # Flash the text if a prophecy is available
        ...
        else: # Dim the text if one is already active
            self.font_locked.render(prophecy_prompt, surface, prophecy_pos)
