...
class Player(Entity):
    def __init__(self, *args):
        ...
        self.jump_rot = 0
        self.air_time = 0
        # <-- NEW FEATURE: Coyote Time & Jump Buffering
        self.coyote_timer = 0
        self.jump_buffer_timer = 0

    def attempt_jump(self):
        # Allow jump if we have jumps left OR if we are in coyote time
        if self.jumps or self.coyote_timer > 0:
            ...
            # Consume a jump only if we weren't in coyote time
            if self.coyote_timer <= 0:
                self.jumps -= 1
                
            self.coyote_timer = 0 # Coyote time is used up
            self.jumping = True
            self.jump_rot = 0
    ...
    def update(self, tiles):
        ...
        if collisions['bottom']:
            ...
            self.velocity[1] = 0
            self.air_time = 0
            # If we land and have a jump buffered, jump!
            if self.jump_buffer_timer > 0:
                self.jump_buffer_timer = 0
                sounds['jump'].play()
                self.attempt_jump()
        else: # In air
            # If we were just on the ground last frame, start coyote time
            if was_on_ground:
                self.coyote_timer = 6 # 1/10th of a second
...
while True:
    ...
    # <-- NEW FEATURE: Screen Shake Update
    if screen_shake > 0:
        screen_shake -= 0.5
    ...
    for i, tile in sorted(enumerate(tile_drops), reverse=True):
        ...
        if r.colliderect(player.rect):
            if not dead:
                sounds['death'].play()
                screen_shake = 20 # <-- NEW FEATURE: Screen Shake
        ...
        if check_pos in tiles:
            ...
            # Shake if block lands near player
        ...
        if collisions['bottom']:
            ...
            if tile_pos_below in tiles:
                ...
                if tile_type_below == 'fragile':
                    ...
                    screen_shake = 10 # <-- NEW FEATURE: Screen Shake
        ...
        if event.type == KEYDOWN:
            ...
            if not dead:
                ...
                if event.key in [K_UP, K_w, K_SPACE]:
                    # <-- NEW FEATURE: Jump Buffering
                    player.jump_buffer_timer = 8
                    if player.air_time < 2 or player.coyote_timer > 0:
                        sounds['jump'].play()
                        player.attempt_jump()
                if event.key in [K_e, K_x]:
                    ...
                    if current_item == 'warp':
                        sounds['warp'].play()
                        screen_shake = 15 # <-- NEW FEATURE: Screen Shake
                    ...
                    if current_item == 'jump':
                        sounds['super_jump'].play()
                        screen_shake = 12 # <-- NEW FEATURE: Screen Shake
                        player.jumps += 1
                        player.attempt_jump()
                        player.velocity[1] = -8
    ...
    # <-- NEW FEATURE: Apply Screen Shake
    render_offset = [0, 0]
    if screen_shake > 0:
