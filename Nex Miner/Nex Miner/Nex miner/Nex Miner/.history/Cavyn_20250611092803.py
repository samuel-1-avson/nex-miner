...
def game_loop():
    ...
    for i in range(WINDOW_TILE_SIZE[1] + 4):
        ...
        display.blit(edge_tile_img, (0 - render_offset[0], pos_y))
        display.blit(edge_tile_img, (TILE_SIZE * (WINDOW_TILE_SIZE[0]-1) - render_offset[0], pos_y))
    ...
    if not dead:
        ...
        if combo_multiplier > 1.0:
            ...
            temp_font_surf = pygame.Surface(white_font.letters[0].get_size(), pygame.SRCALPHA)
            white_font.render_to(combo_text, temp_font_surf, (0, 0))
            scaled_font_surf = pygame.transform.scale(temp_font_surf, (int(width * combo_text_scale), int(white_font.line_height * combo_text_scale)))
            pos = (DISPLAY_SIZE[0] // 2 - scaled_font_surf.get_width() // 2, 8 - (scaled_font_surf.get_height() - white_font.line_height) / 2)
            display.blit(scaled_font_surf, pos)
...
def upgrade_loop():
    ...
    for i, key in enumerate(UPGRADE_DEFINITIONS.keys()):
        ...
        cost_font.render(f"Cost: {cost}", display, (DISPLAY_SIZE[0] - 80 - cost_font.width(f"Cost: {cost}"), y_offset))
        y_offset += 25
