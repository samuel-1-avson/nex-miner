...
BIOMES = [
    {
        ...
        'available_tiles': ['tile', 'chest', 'fragile', 'bounce', 'spike', 'greed', 'magnetic', 'motherlode'], # <-- NEW: Add motherlode here
    ...
    },
    {
        ...
        'available_tiles': ['tile', 'chest', 'fragile', 'bounce', 'spike', 'greed', 'sticky', 'motherlode'], # <-- NEW: And here
...
motherlode_tile_img = load_img('data/images/motherlode_tile.png') # <-- NEW
    ...
    if not perk_selection_active:
        ...
        if master_clock > 180:
            ...
            if game_timer > 10 + 25 * (20000 - min(20000, master_clock)) / 20000:
                ...
                if elite_roll <= 3: # 3% chance for an elite tile
        ...
        for i, tile in sorted(enumerate(tile_drops), reverse=True):
            if freeze_timer <= 0: # Check if time is frozen
                # <-- MODIFIED: Handle falling speed for new tile -->
                fall_speed = 1.4
                if tile[2] == 'motherlode':
                    fall_speed = 1.8 # Faster fall!
            ...
            if check_pos in tiles:
                ...
                # <-- NEW: Handle Motherlode landing -->
                if tile[2] == 'motherlode':
                    tile_drops.pop(i)
                    
                    # 1. Place a NORMAL tile where it landed so the chest has a base
                    ...
                    # 3. Place a chest on top of the newly formed tile
            ...
            # <-- MODIFIED: Drawing logic for airborne tiles -->
            if tile[2] == 'greed':
