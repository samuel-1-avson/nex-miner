...
class MainMenuState(State):
    def __init__(self, game):
        ...
        self.play_options = ["CLASSIC", "ZEN MODE", "HARDCORE", "CHALLENGES", "DAILY CHALLENGE"]
    ...
    def select_option(self):
        ...
        if 'coin' in self.game.sounds: self.game.sounds['coin'].play()
    ...
    def select_submenu_option(self):
        ...
        # Zen and Hardcore modes also go to the Biome Selection screen now
        ...
        elif option == "CHALLENGES": self.game.push_state(ChallengeSelectState(self.game))
        elif option == "DAILY CHALLENGE": self.game.push_state(DailyChallengeState(self.game))
        
        self.in_submenu = False # Always exit submenu after a choice is made
    ...
    def render(self, surface):
        ...
        if self.in_submenu:
             self.render_menu_options(surface, self.play_options, self.submenu_index)
        else:
             self.render_menu_options(surface, self.options, self.selection_index)
        ...
        if coin_icon: surface.blit(coin_icon, (bank_pos[0] - coin_icon.get_width() - 4, bank_pos[1] - 1))
    ...
    def render_menu_options(self, surface, options_list, current_selection_index):
        ...
        for i, option in enumerate(options_list):
            ...
            color = (255, 230, 90) # Highlight color
            w = self.game.white_font.width(option, 2)
            x_pos = surface.get_width()//2 - w//2
            
            # --- NEW: Notification Rendering ---
            notification_text = ""
            if option == "CHARACTERS" and 'characters' in self.game.unlock_notifications:
                notification_text = "!"
            if option == "PLAYER HUB" and 'artifacts' in self.game.unlock_notifications:
                notification_text = "!"
            # ---

            if i == current_selection_index:
                 # Draw the highlight box behind the black text for a better look
                 pygame.draw.rect(surface, color, (x_pos - 10, y_pos - 3, w + 20, 16), border_radius=3)
                 self.game.black_font.render(option, surface, (x_pos, y_pos), 2)
                 if notification_text: # Render notification on top
                     self.highlight_font.render(notification_text, surface, (x_pos + w + 5, y_pos - 2), 2)
            else:
                self.game.white_font.render(option, surface, (x_pos, y_pos), 2)
                if notification_text: # Render notification on top
                     self.game.highlight_font.render(notification_text, surface, (x_pos + w + 5, y_pos - 2), 2)
