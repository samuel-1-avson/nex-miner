...
# -- MODIFIED: Graphics Overhaul with Parallax --
try:
    # Farthest layer: stars, distant grid from 'vaporwave.jpg'
    background_layer_far = pygame.image.load('data/images/background.png').convert()
    background_layer_far = pygame.transform.scale(background_layer_far, DISPLAY_SIZE)
    # Nearer layer: mountains from the original 'background.png'
    background_layer_near = load_img('data/images/background.png') # Uses the game's loader which sets colorkey
    background_layer_near = pygame.transform.scale(background_layer_near, DISPLAY_SIZE)
except pygame.error:
    print("Warning: Could not load all background layers. Parallax effect will be disabled.")
    background_layer_far = None
    background_layer_near = None
...
def handle_death():
    ...
    if player_shielded:
        player_shielded = False
        sounds['explosion'].play() # A sound for the shield breaking
        screen_shake = 15
...
while True:
    ...
    # Base color, in case images fail to load
    display.fill(current_palette['bg'])

    if background_layer_far:
        # Far layer scrolls slowly. `height` is the total upward scroll of the game world.
        # We scroll the background images to give an illusion of camera moving up.
        scroll_y = (height * 0.2) % DISPLAY_SIZE[1]
        display.blit(background_layer_far, (0, scroll_y))
        display.blit(background_layer_far, (0, scroll_y - DISPLAY_SIZE[1]))

    if background_layer_near:
        # Near layer scrolls faster for parallax effect.
        scroll_y = (height * 0.4) % DISPLAY_SIZE[1]
        display.blit(background_layer_near, (0, scroll_y))
        display.blit(background_layer_near, (0, scroll_y - DISPLAY_SIZE[1]))
    ...
    # Inside the `if not dead:` block in the main loop
    
        # ... (after drawing the hud_panel and coin info)
