...
from .character_select_state import CharacterSelectState # <-- IMPORTED
import pygame

class MainMenuState(State):
    def __init__(self, game):
        super().__init__(game)
        self.options = ["PLAY", "CHARACTERS", "UPGRADES", "SETTINGS", "QUIT"] # <-- MODIFIED
    ...
    def render(self, surface):
        ...
        # Options
        y_pos = 80 # <-- Adjusted Y-position for more options
        for i, option in enumerate(self.options):
            ...
            y_pos += 25 # <-- Adjusted spacing between options