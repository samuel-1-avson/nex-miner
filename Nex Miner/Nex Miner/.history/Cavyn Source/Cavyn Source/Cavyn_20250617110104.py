...
class Game:
    ...
    """
    def __init__(self):
        # --- MODIFIED: Load configs and save data BEFORE pygame.init() ---
        self.load_configs()
        ...
        self.SCALE = 4 # Increased default scale for a larger window
        self.TILE_SIZE = 16
        pygame.display.set_caption('Cavyn: Recharged')
        
        # --- MODIFIED: Window mode is now determined by settings ---
        ...
        # --- MODIFIED: Loading order adjusted ---
        self.load_assets()
        self.apply_settings()
        self.upgrade_keys = list(self.upgrades.keys())
        self.unlock_notifications = set()

    # --- NEW: Function to apply window mode setting changes ---
    def update_window_mode(self):
        """Re-initializes the display with the current window mode setting."""
        resizable = self.save_data['settings'].get('resizable_window', False)
        flags = RESIZABLE if resizable else 0
        # Reset the window to the default scaled size with the new flag
    ...
    def load_save(self):
        ...
        try:
            with open(SAVE_FILE, 'r') as f:
                ...
                # --- MODIFIED: Add new setting with a default value ---
                data['settings'].setdefault("resizable_window", False)
                # ... (rest of sanitation is the same)
            ...
            default_save = {
                ...
                "high_score": 0,
                # --- MODIFIED: Add new setting to default save file ---
    ...
    def run(self):
        while True:
            ...
            # --- MODIFIED: Handle resize events if the window is resizable ---
            for event in events:
