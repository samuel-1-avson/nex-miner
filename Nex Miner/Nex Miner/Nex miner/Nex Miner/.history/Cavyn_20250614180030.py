...
class Game:
    ...
    def load_save(self):
        ...
        try:
            ...
            default_save = {
                ...
                "high_score": 0,
                "settings": { "sfx_volume": 1.0, "music_volume": 0.8, "screen_shake": True } # Add defaults here too
            }
    ...
    def apply_settings(self):
        # Apply sound volume
        sfx_vol = self.save_data['settings']['sfx_volume']
        for sound in self.sounds.values():
            # You might have specific volumes already set, so we'll scale by the master sfx volume
            # To do this properly, we should store original volumes.
            # For now, let's just do a simple set. A better implementation is noted below.
            sound.set_volume(sfx_vol)

        # Apply music volume
        music_vol = self.save_data['settings']['music_volume']
        pygame.mixer.music.set_volume(music_vol)

    # In your __init__ after loading save_data, call it:
    # self.save_data = self.load_save()
    # self.apply_settings()
