...
# <-- MODIFIED: This will be wider now
...
vignette_img = load_img('data/images/vignette.png') # <-- NEW
panel_img = load_img('data/images/panel.png') # <-- NEW
...
class Player(Entity):
    ...
    def attempt_jump(self):
        if self.jumps:
            if self.jumps == self.jumps_max: # First jump
                self.velocity[1] = -5
            else: # Double jump
                self.velocity[1] = -4
                ...
                for i in range(24):
                    ...
                    sparks.append([[self.center[0], self.center[1]], [math.cos(angle) * speed, math.sin(angle) * speed], random.random() * 2 + 2, 0.08, (12, 10, 14), True, 0.05])
            self.jumps -= 1
            self.jumping = True
            self.jump_rot = 0
    ...
    def update(self, tiles):
        ...
        if collisions['bottom']:
            if self.air_time > 5: # Only make particles if falling for a bit
                for i in range(4):
...
def reset_game():
    global player, dead, tiles, tile_drops, sparks, game_timer, height, target_height, coins, end_coin_count, current_item, master_clock, last_place, item_used, combo_multiplier, combo_timer, stack_heights, items, camera_pos
...
# --- GAME STATE LOOPS ---
def game_loop():
    global master_clock, game_timer, dead, last_place, height, target_height, combo_timer, combo_multiplier, combo_pop, banked_coins, game_state, end_coin_count, coins, player, tiles, tile_drops, bg_particles, sparks, current_item, item_used, stack_heights, items, COMBO_DURATION, camera_pos
        ...
        if tile_type == 'chest':
            ...
            if player.rect.colliderect(chest_r):
                ...
                if random.randint(1, 10) < 3 + upgrades.get('item_luck', 0):
                    items.append(Item(animation_manager, (chest_r.x + 5, chest_r.y + 5), (6, 6), random.choice(['warp', 'cube', 'jump']), velocity=[random.random() * 5 - 2.5, random.random() * 2 - 5]))
                else:
    ...
    for i in range(WINDOW_TILE_SIZE[0] + 2):
        for j in range(2):
            display.blit(edge_tile_img, (i * TILE_SIZE - (render_offset[0] % TILE_SIZE), -render_offset[1] + j * (DISPLAY_SIZE[1] - TILE_SIZE)))
    ...
    for i, spark in sorted(enumerate(sparks), reverse=True):
        # <-- NEW: Convert tuples to lists to ensure mutability
        spark[0] = list(spark[0])
        spark[1] = list(spark[1])
    
        # This logic correctly handles the different spark formats
        if len(spark) > 5: # Physics-enabled spark
            if len(spark) < 8:
                spark.append(False)
            ...
            if spark[2] <= 0:
                ...
                pygame.draw.circle(display, spark[4], [int(spark[0][0] - render_offset[0]), int(spark[0][1] - render_offset[1])], int(spark[2]))
    # --- UI & VIGNETTE ---
    if not dead:
        ...
        if current_item:
            ...
            display.blit(item_icons[current_item], (DISPLAY_SIZE[0] - 27, 16))
