...
class PlayerHubState(State):
    ...
    def seek_directive(self):
        if not self.game.ai_agent or self.game.ai_agent.is_thinking or self.game.save_data['active_directive']: return
        self.ai_input_active = False
    ...
    def render_mainframe(self, surface, p_rect, y):
        ...
        if not self.game.ai_agent: self.game.white_font.render("Mainframe is offline. (Check API Key)", surface, response_box.topleft, response_box.width)
        ...
        elif self.game.save_data.get('active_directive'): self.game.white_font.render(self.game.save_data['active_directive']['flavor_text'], surface, response_box.topleft, response_box.width)
        ...
        else: self.game.white_font.render("Input query. Ex: 'How to increase energy?' or 'Tell me about the Infiltrator.'", surface, response_box.topleft, response_box.width)
