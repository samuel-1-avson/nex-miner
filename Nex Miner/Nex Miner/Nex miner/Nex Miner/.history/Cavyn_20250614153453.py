...
def load_save():
    try:
        with open(SAVE_FILE, 'r') as f:
            data = json.load(f)
            # Backwards compatibility for new save data fields
            if 'high_score' not in data: 
                data['high_score'] = 0
            if 'upgrades' not in data: # Handle even older files
                 data['upgrades'] = {"jumps": 0, "speed": 0, "item_luck": 0, "coin_magnet": 0}
            if 'coin_magnet' not in data['upgrades']:
                data['upgrades']['coin_magnet'] = 0
            if 'tutorial_completed' not in data:
                data['tutorial_completed'] = False
            if 'codex_discoveries' not in data:
                data['codex_discoveries'] = {"tiles": [], "perks": [], "items": []}
            return data
    except (FileNotFoundError, json.JSONDecodeError):
        # Create a new save file with all fields from the start
        default_save = {
            "banked_coins": 100, 
            "upgrades": {"jumps": 0, "speed": 0, "item_luck": 0, "coin_magnet": 0}, 
            "high_score": 0,
            "tutorial_completed": False,
            "codex_discoveries": {"tiles": [], "perks": [], "items": []}
        }
...
def unlock_codex_entry(category, key):
    """Checks if a key is in the codex, adds it if not, and saves."""
    if key not in save_data['codex_discoveries'][category]:
        save_data['codex_discoveries'][category].append(key)
        write_save(save_data)
        # Optional: you could play a small sound here
        # if 'collect_item' in sounds: sounds['collect_item'].play()
...
BIOMES = [
    {
        ...
        'available_tiles': ['tile', 'chest', 'fragile', 'bounce', 'spike', 'greed', 'magnetic', 'motherlode', 'unstable', 'grav_well'],
    ...
    },
    {
        ...
        'available_tiles': ['tile', 'chest', 'fragile', 'bounce', 'spike', 'greed', 'sticky', 'motherlode', 'unstable', 'grav_well'],
...
motherlode_tile_img = load_img('data/images/motherlode_tile.png')
unstable_tile_img = load_img('data/images/unstable_tile.png')
grav_well_tile_img = load_img('data/images/grav_well_tile.png')
...
class Player(Entity):
    ...
    def update(self, tiles, time_scale=1.0):
        ...
        for tile_drop in tile_drops:
            if tile_drop[2] == 'grav_well':
                if abs(tile_drop[1] - self.pos[1]) < DISPLAY_SIZE[1]:
                    tile_x_center = tile_drop[0] + TILE_SIZE // 2
                    player_x_center = self.center[0]
                    distance = tile_x_center - player_x_center
                    if abs(distance) < 80:
                        pull_force = (distance / 80) * 0.15 
                        self.velocity[0] += pull_force * time_scale
        self.air_time += 1
        ...
        if self.dash_timer > 0:
            ...
            gravity = 0.3
            if 'feather_fall' in active_perks:
                gravity = 0.24
    ...
    if not perk_selection_active:
        ...
        if not dead:
            ...
            if slowing_time:
                ...
                if player.is_charging_dash:
                    player.focus_meter = max(0, player.focus_meter - 1.5)
                    if player.focus_meter <= 0:
        ...
        if master_clock > 180:
            ...
            if game_timer > 10 + 25 * (20000 - min(20000, master_clock)) / 20000:
                ...
                elif elite_roll <= 7: tile_type = 'grav_well'
                else:
                ...
                c = random.choice(options); last_place = c
        ...
        for i, tile in sorted(enumerate(tile_drops), reverse=True):
            ...
            if graze_rect.colliderect(r) and not player.rect.colliderect(r):
                graze_bonus = 0.2
                if 'glass_cannon' in active_perks: graze_bonus *= 2
            ...
            if player.dash_timer > 0 and r.colliderect(player.rect):
                ...
                coins += 2
                dash_bonus = 0.3
                if 'glass_cannon' in active_perks: dash_bonus *= 2
                combo_multiplier += dash_bonus
                combo_timer = COMBO_DURATION
                if 'explosion' in sounds: sounds['explosion'].play()
                else: sounds['block_land'].play()
            ...
            if 'glass_cannon' in active_perks and r.colliderect(player.rect):
                ...
                player_shielded = False
                handle_death()
                player_shielded = temp_shield_state 
            ...
            if check_pos in tiles:
                ...
                if tile[2] == 'motherlode':
                    unlock_codex_entry('tiles', 'motherlode')
                    tile_drops.pop(i)
                    tiles[place_pos] = {'type': 'tile', 'data': {}}
                    stack_heights[place_pos[0] - 1] = place_pos[1]
                    sounds['chest_open'].play(); screen_shake = max(screen_shake, 8)
                    coin_multiplier_perk = 2 if 'greedy' in active_perks else 1
                    for k in range(random.randint(8, 15) * coin_multiplier_perk):
                        items.append(Item(animation_manager, (place_pos[0] * TILE_SIZE + 5, (place_pos[1] -1) * TILE_SIZE + 5), (6, 6), 'coin', velocity=[random.random() * 6 - 3, random.random() * 2 - 8]))
                    tiles[place_pos] = {'type': 'chest', 'data': {}}
                    unlock_codex_entry('tiles', 'chest')
                    recalculate_stack_heights()
                    continue
                    
                if tiles[check_pos]['type'] == 'greed':
                    unlock_codex_entry('tiles', 'greed')
                    tile_drops.pop(i)
                    sounds['chest_open'].play()
                    del tiles[check_pos]
                    coin_multiplier_perk = 1
                    if 'greedy' in active_perks: coin_multiplier_perk = 2
                    for k in range(random.randint(5, 10) * coin_multiplier_perk): 
                ...
                tile_data_dict = {'type': tile[2], 'data': {}}
                if tile[2] == 'unstable':
                    tile_data_dict['data']['timer'] = 180
                elif tile[2] == 'grav_well':
                    tile_data_dict['type'] = 'tile'
                
                unlock_codex_entry('tiles', tile_data_dict['type'])
                tiles[place_pos] = tile_data_dict
            ...
            if tile[2] == 'greed': display.blit(greed_tile_img, (tile[0], tile[1] + height))
            elif tile[2] == 'motherlode': display.blit(motherlode_tile_img, (tile[0], tile[1] + height))
            elif tile[2] == 'unstable': display.blit(unstable_tile_img, (tile[0], tile[1] + height))
            elif tile[2] == 'grav_well': display.blit(grav_well_tile_img, (tile[0], tile[1] + height))
            else: display.blit(tile_img, (tile[0], tile[1] + height))
            if tile[2] == 'chest': display.blit(ghost_chest_img, (tile[0], tile[1] + height - TILE_SIZE))
        ...
        for tile_pos, tile_data in tiles.items():
            if tile_data['type'] == 'unstable':
                if 'timer' in tile_data['data']:
                    tile_data['data']['timer'] -= 1 * time_scale
                    if random.randint(1, 8) == 1: sparks.append([[tile_pos[0] * TILE_SIZE + 8, tile_pos[1] * TILE_SIZE + 8], [random.uniform(-0.5, 0.5), random.uniform(-0.5, 0.5)], random.uniform(2, 4), 0.1, (255, 100, 20), False, 0])
                    if tile_data['data']['timer'] <= 0:
                        if tile_pos not in tiles_to_remove: tiles_to_remove.append(tile_pos)
                        if 'explosion' in sounds: sounds['explosion'].play()
                        screen_shake = max(screen_shake, 12)
                        for y_offset in range(-1, 2):
                            for x_offset in range(-1, 2):
                                neighbor_pos = (tile_pos[0] + x_offset, tile_pos[1] + y_offset)
                                if neighbor_pos in tiles and neighbor_pos not in tiles_to_remove:
                                    tiles_to_remove.append(neighbor_pos)
                        for i_inner in range(40):
                            angle = random.random() * math.pi * 2; speed = random.random() * 2.5
                            sparks.append([[tile_pos[0]*TILE_SIZE+8, tile_pos[1]*TILE_SIZE+8], [math.cos(angle)*speed,math.sin(angle)*speed], random.random()*5+3, 0.08, (255,120,40), True, 0.1])
        ...
        for tile_pos in tiles:
            ...
            if tile_type == 'magnetic': display.blit(magnetic_tile_img, (TILE_SIZE * tile_pos[0], TILE_SIZE * tile_pos[1] + int(height)))
            if tile_type == 'unstable': display.blit(unstable_tile_img, (TILE_SIZE * tile_pos[0], TILE_SIZE * tile_pos[1] + int(height)))
            if tile_type == 'sticky': display.blit(sticky_tile_img, (TILE_SIZE * tile_pos[0], TILE_SIZE * tile_pos[1] + int(height)))
            ...
            if tile_type == 'chest':
                ...
                if chest_r.colliderect(player.rect):
                    sounds['chest_open'].play(); combo_timer = COMBO_DURATION
                    combo_multiplier += 2.0 if 'glass_cannon' in active_perks else 1.0
                    ...
                    if random.randint(1,5) < item_luck_threshold:
                        item_type = random.choice(['warp','cube','jump','bomb','freeze','shield'])
                        items.append(Item(animation_manager, (tile_pos[0]*TILE_SIZE+5, (tile_pos[1]-1)*TILE_SIZE+5),(6,6), item_type, velocity=[random.random()*5-2.5,random.random()*2-5]))
                    else:
                        for i_inner in range(random.randint(2,6) * coin_multiplier_perk):
        ...
        for i, item in sorted(enumerate(items), reverse=True):
            ...
            if item.type=='coin' and not dead and save_data['upgrades']['coin_magnet']>0:
                magnet_level = save_data['upgrades']['coin_magnet']
                pull_radius = 20 + magnet_level*10; pull_strength = 0.05 + magnet_level*0.05
                dist_x, dist_y = player.center[0]-item.center[0], player.center[1]-item.center[1]
            ...
            if item.time > 30 and item.rect.colliderect(player.rect):
                if item.type == 'coin':
                    sounds['coin'].play(); coins += int(1*combo_multiplier); combo_multiplier += 0.2 if 'glass_cannon' in active_perks else 0.1; combo_timer=COMBO_DURATION
                else:
                    sounds['collect_item'].play()
                    unlock_codex_entry('items', item.type)
                    if item.type == 'shield': player_shielded = True
                    else: current_item = item.type
                items.pop(i)
                continue
        ...
        for i, p in sorted(enumerate(projectiles), reverse=True):
            ...
            for j, tile_drop in sorted(enumerate(tile_drops), reverse=True):
                ...
                if p.rect.colliderect(r):
                    sounds['block_land'].play(); p.health -= 1
                    for k in range(15):
                    ...
                    tile_drops.pop(j)
                    if p.health <= 0: projectiles.pop(i); hit_tile = True
                    coins += 1; combo_multiplier += 0.05; combo_timer = COMBO_DURATION
                    break
            if hit_tile: continue

        if not dead:
            for tile_pos, tile_data in tiles.items():
                if tile_data['type'] == 'magnetic' and 0 < TILE_SIZE*tile_pos[1]+int(height) < DISPLAY_SIZE[1]:
                    tile_x_center = tile_pos[0] * TILE_SIZE + TILE_SIZE / 2
                    player_x_center = player.center[0]
                    distance = tile_x_center - player_x_center
                    pull_force = max(-0.25, min(0.25, distance/80))
                    player.velocity[0] += pull_force * time_scale

        if not dead:
            collisions = player.update(tile_drop_rects + edge_rects + tile_rects, time_scale)
            if not collisions['bottom'] and not dead:
                is_sliding = False
                tile_pos_left = ((player.rect.left-2)//TILE_SIZE, player.rect.centery//TILE_SIZE)
                if tile_pos_left in tiles and tiles[tile_pos_left]['type']=='sticky': is_sliding=True
                tile_pos_right = ((player.rect.right+2)//TILE_SIZE, player.rect.centery//TILE_SIZE)
                if tile_pos_right in tiles and tiles[tile_pos_right]['type']=='sticky': is_sliding=True
                if is_sliding: player.velocity[1] = min(player.velocity[1], 1.0)
            if collisions['bottom']:
                ...
                if tile_pos_below in tiles:
                    ...
                    if tile_type_below=='fragile' and 'timer' not in tiles[tile_pos_below]['data']:
                        ...
                        player.velocity[1]=-9; player.jumps=player.jumps_max; sounds['super_jump'].play(); screen_shake=10; combo_multiplier+=0.5; combo_timer=COMBO_DURATION
                    elif tile_type_below=='spike':
                        handle_death(); player.velocity=[0,-4]
        else:
            player.opacity=80; player.update([], time_scale); player.rotation-=16
        ...
        for i, spark in sorted(enumerate(sparks), reverse=True):
            ...
            if spark[5] and (((int(spark[0][0]//TILE_SIZE),int(spark[0][1]//TILE_SIZE))in tiles) or (spark[0][0]<TILE_SIZE) or (spark[0][0]>DISPLAY_SIZE[0]-TILE_SIZE)):
                spark[0][0]-=spark[1][0]; spark[1][0]*=-0.7
            spark[0][1]+=spark[1][1]*time_scale
            if spark[5] and (int(spark[0][0]//TILE_SIZE), int(spark[0][1]//TILE_SIZE)) in tiles:
        ...
        if not dead and not perk_selection_active:
            ...
            if coins>0 and coins//score_interval > last_perk_score//score_interval:
                ...
                if available_perks:
                    ...
                    perk_selection_choice = 0; sounds['warp'].play()

        if not dead:
            hud_panel=pygame.Surface((DISPLAY_SIZE[0], 28)); hud_panel.set_alpha(100); hud_panel.fill((0,0,1)); display.blit(hud_panel,(0,0))
            display.blit(coin_icon, (4,4)); black_font.render(str(coins), display,(17,6)); white_font.render(str(coins), display,(16,5))
            time_bar_pos_x,time_bar_pos_y,time_bar_w,time_bar_h=4,17,40,6; pygame.draw.rect(display,(0,0,1),[time_bar_pos_x,time_bar_pos_y,time_bar_w,time_bar_h])
            bar_fill_width = (time_meter / time_meter_max) * (time_bar_w-2); bar_color=(80,150,255)
            if slowing_time and not player.is_charging_dash and master_clock%10<5: bar_color=(255,255,100)
            ...
            focus_bar_x,focus_bar_y,focus_bar_w,focus_bar_h = DISPLAY_SIZE[0]-25,8,8,16; pygame.draw.rect(display, (0,0,1), [focus_bar_x,focus_bar_y,focus_bar_w,focus_bar_h])
            bar_color = (255,150,40)
            if player.is_charging_dash:
                if master_clock%10<5: bar_color = (255,255,255)
            elif player.focus_meter < player.FOCUS_DASH_COST:
                bar_color = (100,60,20)
            fill_ratio=player.focus_meter/player.FOCUS_METER_MAX; current_fill_h=int(focus_bar_h*fill_ratio); fill_y=(focus_bar_y+focus_bar_h)-current_fill_h
            pygame.draw.rect(display,bar_color,[focus_bar_x+1, fill_y, focus_bar_w-2, current_fill_h])
            if combo_multiplier > 1.0:
                combo_text=f"x{combo_multiplier:.1f}"; scale=2; text_width=white_font.width(combo_text,scale=scale)
                pos_x,pos_y = DISPLAY_SIZE[0]//2-text_width//2, 4
                combo_color=(255,150,40);
                if master_clock%20>10: combo_color=(255,255,255)
                combo_font=Font('data/fonts/small_font.png', combo_color)
                black_font.render(combo_text,display,(pos_x+1,pos_y+1),scale=scale); combo_font.render(combo_text,display,(pos_x,pos_y),scale=scale)
                bar_width = (combo_timer/COMBO_DURATION)*60; bar_x = DISPLAY_SIZE[0]//2 - 30; bar_y=20
                pygame.draw.rect(display, (0,0,1), [bar_x+1,bar_y+1,60,4]); pygame.draw.rect(display,(251,245,239),[bar_x,bar_y,bar_width,4])
            if current_item:
                ...
                if not item_used and ((master_clock%100<80) or (abs(master_clock%100-90)<3)):
                    use_text='E/X'; text_w=white_font.width(use_text)
                    black_font.render(use_text,display,(DISPLAY_SIZE[0]-text_w-23,10)); white_font.render(use_text,display,(DISPLAY_SIZE[0]-text_w-24,9))
            perk_x_offset, perk_y_pos=5, DISPLAY_SIZE[1]-12
            for perk_key in sorted(list(active_perks)):
                if perk_key in perk_icons:
                    ...
                    display.blit(icon_bg, (perk_x_offset-1,perk_y_pos-1)); display.blit(icon,(perk_x_offset,perk_y_pos)); perk_x_offset+=icon.get_width()+4
        else:
            PANEL_COLOR=(15,12,28,220); BORDER_COLOR=(200,200,220,230); TITLE_COLOR=(255,255,255); HIGHLIGHT_COLOR=(255,230,90); AFFORD_COLOR=(140,245,150); CANT_AFFORD_COLOR=(120,120,130); NEW_BEST_COLOR=(100,255,120)
            highlight_font, cant_afford_font, afford_font, new_best_font, title_font = Font('data/fonts/small_font.png', HIGHLIGHT_COLOR), Font('data/fonts/small_font.png', CANT_AFFORD_COLOR), Font('data/fonts/small_font.png', AFFORD_COLOR), Font('data/fonts/small_font.png', NEW_BEST_COLOR), Font('data/fonts/small_font.png', TITLE_COLOR)
            panel_rect = pygame.Rect(10,8,DISPLAY_SIZE[0]-20,DISPLAY_SIZE[1]-16)
            panel_surf=pygame.Surface(panel_rect.size,pygame.SRCALPHA); panel_surf.fill(PANEL_COLOR); display.blit(panel_surf,panel_rect.topleft); pygame.draw.rect(display,BORDER_COLOR,panel_rect,1,border_radius=3)
            title_text="GAME OVER"; text_w=white_font.width(title_text,scale=3)
            black_font.render(title_text,display,(DISPLAY_SIZE[0]//2-text_w//2+2,panel_rect.top+8+2),scale=3); title_font.render(title_text,display,(DISPLAY_SIZE[0]//2-text_w//2,panel_rect.top+8),scale=3)
            line_y1=panel_rect.top+30; pygame.draw.line(display,BORDER_COLOR,(panel_rect.left+10,line_y1),(panel_rect.right-10,line_y1),1)
            score_text,hs_text = f"SCORE: {coins}", f"HIGH SCORE: {save_data['high_score']}"
            score_w,hs_w=white_font.width(score_text,scale=2),white_font.width(hs_text,scale=2)
            black_font.render(score_text,display,(DISPLAY_SIZE[0]/2-score_w/2+1,line_y1+7+1),scale=2); white_font.render(score_text,display,(DISPLAY_SIZE[0]/2-score_w/2,line_y1+7),scale=2)
            hs_font=highlight_font if new_high_score and master_clock%40<20 else white_font
            black_font.render(hs_text,display,(DISPLAY_SIZE[0]/2-hs_w/2+1,line_y1+22+1),scale=2); hs_font.render(hs_text,display,(DISPLAY_SIZE[0]/2-hs_w/2,line_y1+22),scale=2)
            if new_high_score and master_clock%30<20: new_best_font.render("NEW!",display,(DISPLAY_SIZE[0]/2+hs_w/2+5,line_y1+22),scale=2)
            line_y2=line_y1+42; pygame.draw.line(display,BORDER_COLOR,(panel_rect.left+10,line_y2),(panel_rect.right-10,line_y2),1)
            ...
            if run_coins_banked:
                upgrade_title_pos=(panel_rect.left+15,line_y2+7); black_font.render("Upgrades",display,(upgrade_title_pos[0]+1,upgrade_title_pos[1]+1),scale=2); white_font.render("Upgrades",display,upgrade_title_pos,scale=2)
                bank_text=f"Bank: {save_data['banked_coins']}"; bank_w=white_font.width(bank_text); bank_pos=(panel_rect.right-bank_w-15,line_y2+9); display.blit(coin_icon,(bank_pos[0]-coin_icon.get_width()-3,bank_pos[1]-1)); black_font.render(bank_text,display,(bank_pos[0]+1,bank_pos[1]+1)); white_font.render(bank_text,display,bank_pos)
                list_y=line_y2+28
                for i,key in enumerate(upgrade_keys):
                    upgrade,level=UPGRADES[key],save_data['upgrades'][key]
                    name_text,level_text = f"{upgrade['name']}",f"Lvl {level}";
                    if level>=upgrade['max_level']: level_text="MAX"
                    font=highlight_font if i==upgrade_selection else white_font
                    if i==upgrade_selection: pygame.draw.rect(display,(40,30,60,150),[panel_rect.left+5,list_y-2,panel_rect.width//2-10,11],0)
                    font.render(name_text,display,(panel_rect.left+10,list_y)); font.render(level_text,display,(panel_rect.left+80,list_y)); list_y+=13
                detail_x,detail_y=panel_rect.centerx+5,line_y2+28
                sel_key=upgrade_keys[upgrade_selection]; sel_upgrade,sel_level=UPGRADES[sel_key],save_data['upgrades'][sel_key]
                cost=int(sel_upgrade['base_cost']*(1.5**sel_level)); can_afford=save_data['banked_coins']>=cost
                highlight_font.render(sel_upgrade['name'],display,(detail_x,detail_y)); white_font.render(sel_upgrade['desc'],display,(detail_x,detail_y+12))
                if sel_level<sel_upgrade['max_level']:
                    cost_font=afford_font if can_afford else cant_afford_font
                    cost_text,buy_text,cost_pos_y=f"{cost}","[Buy]",detail_y+26
                    display.blit(coin_icon,(detail_x,cost_pos_y)); cost_font.render(cost_text,display,(detail_x+coin_icon.get_width()+4,cost_pos_y)); cost_font.render(buy_text,display,(panel_rect.right-cost_font.width(buy_text)-15,cost_pos_y))
                else: white_font.render("Max Level Reached",display,(detail_x,detail_y+26))
            else:
                banking_text="Banking coins..."; text_w=white_font.width(banking_text); black_font.render(banking_text,display,(DISPLAY_SIZE[0]//2-text_w//2+1,line_y2+25+1)); white_font.render(banking_text,display,(DISPLAY_SIZE[0]//2-text_w//2,line_y2+25))
            prompt_text="Up/Down: Select  -  Enter: Buy  -  R: Restart"; prompt_w=white_font.width(prompt_text); black_font.render(prompt_text,display,(DISPLAY_SIZE[0]//2-prompt_w//2+1,panel_rect.bottom-12+1)); white_font.render(prompt_text,display,(DISPLAY_SIZE[0]//2-prompt_w//2,panel_rect.bottom-12))

    else: # perk_selection_active is True
        display.blit(display_copy, (0, 0))
        overlay = pygame.Surface(DISPLAY_SIZE, pygame.SRCALPHA); overlay.fill((10,5,20,200)); display.blit(overlay, (0,0))
        title_text = "CHOOSE A PERK"
        text_w = white_font.width(title_text, scale=2); title_pos = (DISPLAY_SIZE[0]//2-text_w//2, 20)
        black_font.render(title_text, display, (title_pos[0]+1, title_pos[1]+1), scale=2); white_font.render(title_text, display, title_pos, scale=2)
        ...
        for i, perk_key in enumerate(perks_to_offer):
            ...
            pygame.draw.rect(display,(255,230,90)if i==perk_selection_choice else(150,150,180), panel_rect,1)
            perk_name,perk_desc = PERKS[perk_key]['name'].upper(), PERKS[perk_key]['desc']
            white_font.render(perk_name, display, (panel_rect.x+8, panel_rect.y+7))
            black_font.render(perk_desc, display, (panel_rect.x+9, panel_rect.y+19)); white_font.render(perk_desc, display, (panel_rect.x+8, panel_rect.y+18))

    for event in pygame.event.get():
        if event.type == QUIT: pygame.quit(); sys.exit()
        if event.type == KEYDOWN:
            if perk_selection_active:
                if event.key in [K_DOWN, K_s]: perk_selection_choice = (perk_selection_choice + 1) % len(perks_to_offer)
                if event.key in [K_UP, K_w]: perk_selection_choice = (perk_selection_choice - 1 + len(perks_to_offer)) % len(perks_to_offer)
                if event.key in [K_RETURN, K_e, K_x]:
                    chosen_perk = perks_to_offer[perk_selection_choice]; unlock_codex_entry('perks', chosen_perk); active_perks.add(chosen_perk); perk_selection_active=False
                    if 'upgrade' in sounds: sounds['upgrade'].play()
                continue
            if event.key == K_ESCAPE: pygame.quit(); sys.exit()
            if not dead:
                if event.key in [K_c, K_l] and player.focus_meter >= player.FOCUS_DASH_COST: player.is_charging_dash = True
                ...
                if event.key in [K_z, K_k] and len(projectiles)<4:
                    sounds['jump'].play(); pressed_keys=pygame.key.get_pressed(); proj_speed=4; vel=[0,0]
                    if pressed_keys[K_UP] or pressed_keys[K_w]: vel, proj_pos = [0, -proj_speed], [player.rect.centerx, player.rect.top]
                    elif pressed_keys[K_DOWN] or pressed_keys[K_s]: vel, proj_pos = [0, proj_speed], [player.rect.centerx, player.rect.bottom]
                    elif player.flip[0]: vel, proj_pos = [proj_speed, 0], [player.rect.right, player.rect.centery]
                    else: vel, proj_pos = [-proj_speed, 0], [player.rect.left, player.rect.centery]
                ...
                if event.key in [K_e, K_x] and current_item:
                    item_used=True; item_type = current_item
                    if item_type=='warp':
                        sounds['warp'].play(); screen_shake=15; max_point=min(enumerate(stack_heights),key=lambda x:x[1]); player.pos[0]=(max_point[0]+1)*TILE_SIZE+4; player.pos[1]=(max_point[1]-1)*TILE_SIZE
                    elif item_type=='jump': sounds['super_jump'].play(); screen_shake=12; player.jumps+=1; player.attempt_jump(); player.velocity[1]=-8
                    elif item_type=='cube':
                        sounds['block_land'].play(); place_pos=(int(player.center[0]//TILE_SIZE),int(player.pos[1]//TILE_SIZE)+1)
                        stack_heights[place_pos[0]-1]=place_pos[1]
                        current_base_row=max(tiles,key=lambda x:x[1])[1]-1 if tiles else WINDOW_TILE_SIZE[1]-2
                        for i in range(place_pos[1],current_base_row+2): tiles[(place_pos[0],i)]={'type':'placed_tile','data':{}}
                    elif item_type=='bomb':
                        if 'explosion' in sounds: sounds['explosion'].play()
                        screen_shake=25; tiles_to_bomb=[]; bomb_radius=4*TILE_SIZE
                        for tile_pos in tiles:
                            if math.sqrt((player.center[0]-(tile_pos[0]*TILE_SIZE+TILE_SIZE//2))**2+(player.center[1]-(tile_pos[1]*TILE_SIZE+TILE_SIZE//2))**2)<bomb_radius: tiles_to_bomb.append(list(tile_pos))
                        for tile_pos in tiles_to_bomb:
                            if tuple(tile_pos) in tiles: del tiles[tuple(tile_pos)]
                        recalculate_stack_heights()
                    elif item_type=='freeze': sounds['warp'].play(); screen_shake=10; freeze_timer=360
                    current_item = None
            else: # Dead Controls
                if run_coins_banked:
                    ...
                    if event.key==K_RETURN:
                        key=upgrade_keys[upgrade_selection]; upgrade=UPGRADES[key]; level=save_data['upgrades'][key]
                        cost = int(upgrade['base_cost']*(1.5**level))
                        if level<upgrade['max_level'] and save_data['banked_coins']>=cost:
                            save_data['banked_coins']-=cost; save_data['upgrades'][key]+=1; write_save(save_data)
                            if 'upgrade' in sounds: sounds['upgrade'].play()
            if event.key == K_r and dead:
                ...
                player.speed += save_data['upgrades']['speed'] * 0.08
                player.jumps = player.jumps_max
                dead, tiles, tile_drops, sparks, projectiles, freeze_timer, game_timer, height, target_height, coins, end_coin_count, current_item, master_clock, slowing_time, time_meter, combo_multiplier, combo_timer, run_coins_banked, screen_shake, current_biome_index, player_shielded, new_high_score, perk_selection_active, last_perk_score = False,{},[],[],[],0,0,0,0,0,0,None,0,False,time_meter_max,1.0,0,False,0,0,False,False,False,0
                active_perks.clear()
                for i_inner in range(WINDOW_TILE_SIZE[0]-2): tiles[(i_inner+1, WINDOW_TILE_SIZE[1]-1)] = {'type':'tile','data':{}}
                tiles[(1, WINDOW_TILE_SIZE[1]-2)]={'type':'tile','data':{}}; tiles[(WINDOW_TILE_SIZE[0]-2, WINDOW_TILE_SIZE[1]-2)] = {'type':'tile','data':{}}
                stack_heights=[WINDOW_TILE_SIZE[1]-1 for i_inner in range(WINDOW_TILE_SIZE[0]-2)]; stack_heights[0]-=1; stack_heights[-1]-=1; items=[]
        if event.type == KEYUP:
            if not dead:
                if event.key in [K_c, K_l] and player.is_charging_dash:
                    player.is_charging_dash = False; player.focus_meter -= player.FOCUS_DASH_COST; player.dash_timer = player.DASH_DURATION
                    if 'explosion' in sounds: sounds['explosion'].play()
                    else: sounds['block_land'].play()
