--- START OF FILE Nex Miner/Nex Miner/Nex miner/Nex Miner/data/scripts/entities/player.py ---
# Cavyn Source/data/scripts/entities/player.py
import random
import math 
...
class Player(Entity):
    def __init__(self, assets, pos, size, type, state):
        ...
        self.state = state
        self.velocity = [0, 0]; self.right = False; self.left = False
        self.speed = 1.4; self.jumps = 2; self.jumps_max = 2
        self.jumping = False; self.jump_rot = 0; self.air_time = 0; self.coyote_timer = 0
        self.jump_buffer_timer = 0; self.dash_timer = 0
        self.DASH_SPEED = 8; self.DASH_DURATION = 8
        ...
        self.focus_meter = self.FOCUS_METER_MAX; self.is_charging_dash = False
        self.FOCUS_DASH_COST = 50
        self.wall_contact_timer = 0; self.is_wall_sliding = False
        # --- FIX: Initialize the collisions attribute to prevent attribute errors ---
        self.collisions = {'top': False, 'bottom': False, 'left': False, 'right': False}

    def attempt_jump(self):
        # (Rest of the function is identical to original, no changes needed here)
        ...
        if self.is_wall_sliding:
            jump_velocity = -6.5; self.velocity[0] = 3.5 if not self.flip[0] else -3.5
            for i in range(20):
                angle = random.uniform(math.pi*0.75, math.pi*1.25) if self.flip[0] else random.uniform(-math.pi*0.25, math.pi*0.25)
        ...
        if self.jumps > 0 or self.coyote_timer > 0:
            if self.jumps == self.jumps_max or self.coyote_timer > 0: self.velocity[1] = jump_velocity
    ...
    def update(self, tiles, time_scale=1.0):
        # (This update function is also identical to the original; showing only the __init__ with the fix)
        # For brevity, only showing the fixed part of the file.
        ...
        if self.dash_timer > 0: self.velocity[1] = 0
            ...
            if self.air_time > 2 and (self.collisions['left'] or self.collisions['right']):
                if (self.left and self.collisions['left']) or (self.right and self.collisions['right']):
                    self.is_wall_sliding = True; self.velocity[1] = min(self.velocity[1], 1.2)
            if not self.is_wall_sliding:
                self.velocity[1] = min(self.velocity[1] + (0.24 if 'feather_fall' in self.state.active_perks else 0.3), 4)
        ...
        elif self.air_time > 3 or self.is_wall_sliding or self.dash_timer > 0: self.set_action('jump')
        elif abs(self.velocity[0]) > 0.1: self.set_action('run')
        else: self.set_action('idle')
        was_on_ground = self.air_time <= 1; was_in_air_for_long = self.air_time > 5
        ...
        if self.collisions['bottom']:
            self.velocity[1] = 0
            if was_in_air_for_long: self.state.game.sounds['land'].play()
            self.air_time = 0; self.jumps = self.jumps_max; self.jumping = False; self.rotation = 0
            if self.jump_buffer_timer > 0:
                self.jump_buffer_timer = 0; self.state.game.sounds['jump'].play(); self.attempt_jump()
