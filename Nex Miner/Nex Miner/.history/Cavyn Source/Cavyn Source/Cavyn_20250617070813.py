...
class Game:
    ...
    def __init__(self):
        ...
        # --- NEW: AI Agent Initialization (Secure) ---
        # IMPORTANT: Load your API key from an environment variable for security.
        # Do NOT hardcode your API key in the source code.
        # Example of setting an environment variable (in your terminal/command prompt):
        # export GEMINI_API_KEY="your_actual_api_key_here"  (for Linux/macOS)
        # set GEMINI_API_KEY="your_actual_api_key_here"     (for Windows)
        try:
            api_key = os.environ.get("GEMINI_API_KEY")
            if not api_key:
                print("\n--- WARNING: GEMINI_API_KEY environment variable not set. ---")
                print("The Oracle will be unavailable. Please set the environment variable to use AI features.\n")
                self.ai_agent = None
            else:
                self.ai_agent = GeminiAgent(api_key=api_key) 
        except Exception as e:
            print(f"Error initializing Gemini Agent: {e}")
            self.ai_agent = None # Handle case where initialization fails
        # State Management
        self.states = []
        ...
        self.original_volumes = {
            ...
            'warp': 1.0, 'land': 0.4,
            'prophecy_complete': 1.0, 'artifact_equip': 0.7 # --- NEW ---
        }
