--- START OF FILE Nex Miner/Nex Miner/Nex Miner/Nex miner/Nex Miner/Nex Miner.py ---
...
class Game:
    ...
    def __init__(self):
        ...
        # --- FIX: Read resizable setting from save data for window creation ---
        ...
        try:
            ...
            print(f"Error: {e}")
            print(f"Please ensure you have a valid Gemini API key in a .env file.")
            self.ai_agent = None 
    ...
    # --- NEW: Function to dynamically change the window mode ---
    def update_window_mode(self):
        """Toggles the window between fixed-size and resizable."""
    ...
    def load_dynamic_character(self):
        """Loads the AI-generated character from the save file into the game's active character list."""
    ...
    def return_to_main_menu(self):
        ...
        while self.states and not isinstance(self.get_current_state(), MainMenuState):
            self.states[-1].exit_state()
            self.states.pop()
        # If the main menu still exists on the stack, call its enter_state again to refresh it
    ...
    def restart_gameplay(self):
        ...
        self.push_state(GameplayState(self)) # --- BUG FIX: Pass `self`, not `self.game`


if __name__ == "__main__":
    game = Game()
    # --- Start with the new boot up sequence ---
