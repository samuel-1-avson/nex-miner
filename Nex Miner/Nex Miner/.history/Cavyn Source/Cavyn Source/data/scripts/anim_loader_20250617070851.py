...
class AnimationData:
    def __init__(self, path, colorkey=None):
        ...
        for img_filename in os.listdir(path):
            if img_filename.endswith('.png'):
                ...
                try:
                    ...
                    print(f"Warning: Could not parse frame number from '{img_filename}' in '{path}'. Skipping.")
        ...
        try:
            ...
            try:
                with open(config_path, 'w') as f:
                 ...
                 print(f"Warning: Could not write default config to {config_path}")
        ...
        # --- FIX: Ensure config 'frames' matches number of images to prevent crashes ---
        num_frames_in_config = len(self.config.get('frames', []))
        if num_frames_in_config != len(self.image_list):
            print(f"Warning: Mismatch in '{self.id}'. Found {len(self.image_list)} images but config specifies {num_frames_in_config} frame durations. Adjusting config.")
            self.config['frames'] = [5] * len(self.image_list)
    ...
    @property
    def duration(self):
        return sum(self.config.get('frames', [0]))

class Animation:
    def __init__(self, animation_data):
        self.data = animation_data
        self.frame = 0
        self.paused = self.data.config.get('paused', False)
    ...
    def render(self, surf, pos, offset=(0, 0)):
        ...
        if self.rotation:
            ...
            if not self.data.config.get('centered', False):
    ...
    def play(self, dt=1/60, time_scale=1.0): # Using dt is better than assuming 60fps
        ...
        if self.data.config.get('loop', False):
            ...
            anim_duration = self.data.duration
            if anim_duration > 0:
                while self.frame > anim_duration:
                    self.frame -= anim_duration
                    self.just_looped = True
            else:
                self.frame = 0 # If duration is 0, just reset the frame
        else:
            # Clamp frame to max duration if not looping
            self.frame = min(self.frame, self.data.duration)
...
class AnimationManager:
    ...
    def new(self, anim_id):
        if anim_id in self.animations:
            ...
            # Return a "null" animation to prevent crashes
            # A completely empty directory to serve as a Null object pattern.
            null_path = os.path.join(os.path.dirname(__file__), '..', '..', '..') 
            return Animation(AnimationData(null_path, colorkey=COLORKEY))