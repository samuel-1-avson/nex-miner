...
class CurseSelectionState(State):
    ...
    def reroll_curses(self):
        if self.gameplay_state.curse_rerolls_left <= 0:
            return

        self.gameplay_state.curse_rerolls_left -= 1
    ...
    def render(self, surface):
        ...
        title = "HAZARDOUS PROTOCOL DETECTED"
        ...
        if self.gameplay_state.curse_rerolls_left > 0:
            prompt = f"[R] Reroute Power ({self.gameplay_state.curse_rerolls_left} left)"
