...
class CurseSelectionState(State):
    ...
    def enter_state(self):
        ...
        super().enter_state()
        # Check if we should offer a curse. It's offered every 2 perks.
        if self.perks_gained_this_run > 0 and self.perks_gained_this_run % 2 == 0:
            # last_curse_check prevents offering the same curse screen twice if the player enters/exits pause menu
            if self.perks_gained_this_run != self.last_curse_check:
                self.last_curse_check = self.perks_gained_this_run
                
                # Logic to create and push the curse selection state
