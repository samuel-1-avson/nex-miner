...
# ... (keep all your existing image, sound, and font loading here)
...
# ... (keep the normalize, Item, and Player classes here, UNCHANGED)
...
# ... (keep lookup_nearby and glow_img functions here, UNCHANGED)
...
# <-- NEW: Define global variables that need to be accessed by functions
player = None
dead = False
tiles = {}
tile_drops = []
...
# <-- NEW: State and Save Data Variables
game_state = 'playing' # 'playing', 'game_over', 'upgrades'
banked_coins = 0
upgrades = {}
game_over_selection = 0 # 0 for restart, 1 for upgrades
...
def reset_game():
    global player, dead, tiles, tile_drops, bg_particles, sparks, game_timer, height, target_height, coins, end_coin_count, current_item, master_clock, last_place, item_used, combo_multiplier, combo_timer, stack_heights, items

    player = Player(animation_manager, (DISPLAY_SIZE[0] // 2 - 5, -20), (8, 16), 'player')
    
    # <-- NEW: Apply upgrades to the player
    ...
    tiles = {}
    tile_drops = []
    # bg_particles can persist for a smoother transition
    sparks = []
    game_timer = 0
    height = 0
    target_height = 0
...
def game_loop():
    global master_clock, game_timer, dead, last_place, height, target_height, combo_timer, combo_multiplier, banked_coins, game_state, end_coin_count, coins

    # This function contains the entire `while True` loop's content from before
    ...
    # ... (All the rendering and game logic from the old `while True` loop goes here)
    # I'll just paste it all in for simplicity.
    ...
    for i, tile in sorted(enumerate(tile_drops), reverse=True):
        ...
        if r.colliderect(player.rect):
            if not dead:
                ...
                dead = True
                if end_coin_count == coins: # <-- NEW: Logic to bank coins once
                    banked_coins += coins
                    save_progress()
    ...
    for tile_pos in tiles:
        ...
        if tile_type == 'chest':
            ...
            if chest_r.colliderect(player.rect):
                ...
                if random.randint(1, 5) < 3 + upgrades.get('item_luck', 0):
            ...
            if tile_pos_below in tiles:
                ...
                if tile_type_below == 'fragile':
                    ...
                    for i in range(40):
                        ...
                        sparks.append([ [player.rect.centerx, player.rect.bottom], [math.cos(angle) * speed, math.sin(angle) * speed], random.random() * 3 + 2, 0.04, (8, 2, 12), True, 0.1 ])
    ...
    if player: # player might not exist yet on first frame
        player.render(display, (0, -int(height)))

    # ... (the rest of the rendering, sparks, UI, etc. from the old loop goes here)
    ...
    elif game_state == 'playing': # Show coin count animation only if we are in the playing state but dead
        # ... logic for the old game over screen is now obsolete
        if master_clock % 3 == 0:
            if end_coin_count != coins:
                sounds['coin_end'].play()
            end_coin_count = min(end_coin_count + 1, coins)
        
        # This part is tricky. Let's simplify. When the player dies, we immediately bank the coins.
        # The new 'game_over' state will handle the display. We can remove the old display logic.

# --- NEW: Main Control Loop ---
load_progress()
reset_game()
...
while True:
    if game_state == 'playing':
                ...
                if not dead:
                    ...
                    if event.key in [K_e, K_x]:
                        ...
                        if current_item == 'warp':
                            ...
                            for i in range(60):
                                angle = random.random() * math.pi * 2
                                speed = random.random() * 3; physics_on = random.choice([False, True])
                            ...
                            for i in range(60):
                                angle = random.random() * math.pi * 2
                                speed = random.random() * 1.75; physics_on = random.choice([False, True, True])
                                sparks.append([player.center.copy(), [math.cos(angle) * speed, math.sin(angle) * speed], random.random() * 3 + 3, 0.02, (12, 8, 2), physics_on, 0.1 * physics_on])
                        if current_item == 'jump':
                            ...
                            for i in range(60):
                                ...
                                if random.randint(1, 5) == 1: angle = -math.pi / 2
                                speed = random.random() * 3; physics_on = random.choice([False, True])
                                sparks.append([player.center.copy(), [math.cos(angle) * speed, math.sin(angle) * speed], random.random() * 3 + 3, 0.02, (12, 8, 2), physics_on, 0.1 * physics_on])
        ...
        display.fill((22, 19, 40)) # Black background
        ...
        upgrades_text = "Upgrades"
        
        # Highlighted color
        h_color = (251, 245, 239) # White
        nh_color = (100, 100, 120) # Gray
        ...
        restart_font.render(restart_text, display, (DISPLAY_SIZE[0] // 2 - restart_font.width(restart_text) // 2, 90))
        upgrades_font.render(upgrades_text, display, (DISPLAY_SIZE[0] // 2 - upgrades_font.width(upgrades_text) // 2, 105))
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                if event.key in [K_UP, K_DOWN, K_w, K_s]:
                    game_over_selection = 1 - game_over_selection # Flips between 0 and 1
                if event.key in [K_RETURN, K_SPACE, K_e, K_x]:
                    if game_over_selection == 0: # Restart
                        reset_game()
                        game_state = 'playing'
                    else: # Upgrades
                        # game_state = 'upgrades' # We will implement this next
                        print("Upgrade menu not implemented yet!")


    # This is where the game_state == 'upgrades' logic will go
