...
BIOMES = [
    {
        ...
        'available_tiles': ['tile', 'chest', 'fragile', 'bounce', 'spike', 'greed', 'magnetic', 'motherlode', 'unstable', 'grav_well'], # <-- MODIFIED
    ...
    },
    {
        ...
        'available_tiles': ['tile', 'chest', 'fragile', 'bounce', 'spike', 'greed', 'sticky', 'motherlode', 'unstable', 'grav_well'], # <-- MODIFIED
...
grav_well_tile_img = load_img('data/images/grav_well_tile.png') # <-- NEW
    ...
    if not perk_selection_active:
        ...
        if master_clock > 180:
            ...
            if game_timer > 10 + 25 * (20000 - min(20000, master_clock)) / 20000:
                ...
                # <-- MODIFIED: Tile Spawning Logic -->
                tile_type = 'tile'
                elite_roll = random.randint(1, 100)
                if elite_roll <= 2: 
                    tile_type = 'motherlode'
                elif elite_roll <= 5: 
                    tile_type = 'unstable'
                elif elite_roll <= 7:
                    tile_type = 'grav_well'
        ...
        for i, tile in sorted(enumerate(tile_drops), reverse=True):
            ...
            if check_pos in tiles:
                ...
                elif tile[2] == 'grav_well': # Grav well becomes a normal tile on land
            ...
            elif tile[2] == 'motherlode':
                 display.blit(motherlode_tile_img, (tile[0], tile[1] + height))
            elif tile[2] == 'unstable':
                 display.blit(unstable_tile_img, (tile[0], tile[1] + height))
            elif tile[2] == 'grav_well': # <-- NEW
                 display.blit(grav_well_tile_img, (tile[0], tile[1] + height))
            else:
            ...
            if tile_data['type'] == 'unstable':
                if 'timer' in tile_data['data']:
                    ...
                    if random.randint(1, 8) == 1:
                        sparks.append([[tile_pos[0] * TILE_SIZE + 8, tile_pos[1] * TILE_SIZE + 8], [random.uniform(-0.5, 0.5), random.uniform(-0.5, 0.5)], random.uniform(2, 4), 0.1, (255, 100, 20), False, 0])
        ...
        for tile_pos in tiles:
            ...
            if tile_type == 'unstable':
                display.blit(unstable_tile_img, (TILE_SIZE * tile_pos[0], TILE_SIZE * tile_pos[1] + int(height)))
            if tile_type == 'sticky':
            ...
            if tile_type == 'chest':
                ...
                if chest_r.colliderect(player.rect):
                    sounds['chest_open'].play()
                    chest_bonus = 1.0
                    if 'glass_cannon' in active_perks: chest_bonus *= 2
        ...
        # No more changes below this line for the new features.

        # ... (THE REST OF THE FILE IS IDENTICAL TO THE PREVIOUS VERSION)
        # ... from `base_row = max(tiles...` all the way to the end ...
