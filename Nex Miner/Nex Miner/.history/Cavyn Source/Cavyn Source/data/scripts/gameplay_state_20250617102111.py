...
class GameplayState(State):
    ...
    def handle_death(self):
        ...
        if self.mode == "zen":
            self.player.pos[1] -= 20; self.player.velocity[1] = -5
    ...
    def update_ambient_hazards(self):
        if 'ambient_hazard' in self.game.biomes[self.current_biome_index]:
            ...
            if hazard['type'] == 'rising_lava': self.lava_y -= hazard['speed'] * self.time_scale
    ...
    def update_special_entities(self):
        ...
        for i, crystal_rect in sorted(enumerate(self.crystals), reverse=True):
            crystal_render_rect = pygame.Rect(crystal_rect.x, crystal_rect.y - self.height, crystal_rect.width, crystal_rect.height)
            if self.player.dash_timer > 0 and self.player.rect.colliderect(crystal_render_rect):
                self.crystals.pop(i); self.coins += 5 * int(self.combo_multiplier)
                self.combo_multiplier += 0.5 * (2 if 'glass_cannon' in self.active_perks else 1); self.combo_timer = self.game.COMBO_DURATION
                ...
                for k in range(20):
                    ...
                    self.sparks.append([list(crystal_render_rect.center), [math.cos(angle) * speed, math.sin(angle) * speed], self.random.uniform(2, 5), 0.08, (200, 220, 255), True, 0.1])
                continue

            if self.freeze_timer <= 0: crystal_rect.y += 1.6 * self.time_scale
            if self.player.rect.colliderect(crystal_render_rect): self.crystals.pop(i); self.handle_death()
            
            check_pos_real_world = (int(crystal_rect.centerx // self.game.TILE_SIZE), int(math.floor(crystal_rect.bottom / self.game.TILE_SIZE)))
            if check_pos_real_world in self.tiles:
                ...
                for k in range(self.random.randint(4, 8)):
                    self.items.append(Item(self.game.animation_manager, (crystal_rect.centerx, crystal_rect.bottom), (6, 6), 'coin', self, velocity=[self.random.random() * 4 - 2, self.random.random() * 2 - 6]))
        ...
        if self.combo_timer > 0:
            ...
            if self.combo_timer <= 0 and self.combo_multiplier > 1.0:
                ...
                if shield_level > 0 and not self.combo_shield_used:
                    self.combo_shield_used = True; self.combo_multiplier = 1.0 + shield_level * 1.5
    ...
    def update_prophecy(self):
        if not self.prophecy or self.prophecy.get('completed'): return
    ...
    def update_tile_spawning(self):
        ...
        if self.game_timer > current_spawn_rate:
            ...
            biome = self.game.biomes[self.current_biome_index]; final_tile_type = 'tile' 
    ...
    def update_falling_tiles(self):
        ...
        for i, tile in sorted(enumerate(self.tile_drops), reverse=True):
            ...
            r_real_world = pygame.Rect(tile[0], tile[1], self.game.TILE_SIZE, self.game.TILE_SIZE); r = pygame.Rect(tile[0], tile[1] - self.height, self.game.TILE_SIZE, self.game.TILE_SIZE)
            ...
            if graze_r.colliderect(r) and not self.player.rect.colliderect(r):
                self.combo_multiplier += 0.2 * (2 if 'glass_cannon' in self.active_perks else 1); self.combo_timer = self.game.COMBO_DURATION
            if self.player.dash_timer > 0 and r.colliderect(self.player.rect):
                self.tile_drops.pop(i)
                if 'brittle_blocks' not in self.active_curses: self.coins += 2 * int(self.combo_multiplier)
            ...
            if check_pos_real in self.tiles:
                self.tile_drops.pop(i); place_pos = (check_pos_real[0], check_pos_real[1] - 1)
                if place_pos[1] < 0: continue
                self.tiles[place_pos] = {'type': tile[2], 'data': {}}; self.game.sounds['block_land'].play()
    ...
    def update_placed_tiles(self):
        ...
        for tile_pos, tile_data in self.tiles.items():
            player_on_tile_rect = pygame.Rect(tile_pos[0]*16, tile_pos[1]*16-2-self.height, 16, 18); player_on_tile = self.player.rect.colliderect(player_on_tile_rect)
            ...
            if tile_data['type'] == 'fragile' and 'timer' in tile_data['data']:
                tile_data['data']['timer'] -= 1 * self.time_scale
                if self.random.randint(1, 10) == 1: self.sparks.append([[tile_pos[0] * self.game.TILE_SIZE + self.random.random() * 16, tile_pos[1] * self.game.TILE_SIZE + 14], [self.random.random() * 0.5 - 0.25, self.random.random() * 0.5], self.random.random() * 2 + 1, 0.08, (6, 4, 1), True, 0.05])
                if tile_data['data']['timer'] <= 0:
                    if tile_pos not in to_remove: to_remove.append(tile_pos)
                    for _ in range(20):
                        angle, speed = self.random.random() * math.pi * 2, self.random.random() * 1.5
                        self.sparks.append([[tile_pos[0] * self.game.TILE_SIZE + 8, tile_pos[1] * self.game.TILE_SIZE + 8], [math.cos(angle) * speed, math.sin(angle) * speed], self.random.random() * 4 + 2, 0.05, (6, 4, 1), True, 0.1])
            if tile_data['type'] == 'conduit' and 'static_shock' in self.active_curses:
                ...
                if player_on_tile:
                    ...
                    if tile_data['data']['timer'] > 120:
                        self.handle_death(); tile_data['data']['timer'] = -120
                elif tile_data['data']['timer'] > 0: tile_data['data']['timer'] = 0
                elif tile_data['data']['timer'] < 0: tile_data['data']['timer'] += 1 * self.time_scale
        if to_remove:
            for p in to_remove:
                if p in self.tiles: del self.tiles[p]
                if self.random.random() < self.tile_coin_drop_chance:
                     self.items.append(Item(self.game.animation_manager, (p[0] * 16 + 5, p[1] * 16 + 5), (6, 6), 'coin', self, velocity=[self.random.random() * 2 - 1, self.random.random() * -2]))
            self.recalculate_stack_heights()
    ...
    def update_tile_interactions(self):
        ...
        for pos, data in list(self.tiles.items()):
            if data['type'] == 'chest':
                chest_pos = (pos[0]*self.game.TILE_SIZE, (pos[1]-1)*self.game.TILE_SIZE); r = pygame.Rect(chest_pos[0]+2, chest_pos[1]+6, self.game.TILE_SIZE-4, self.game.TILE_SIZE-6)
                if r.colliderect(self.player.rect):
                    self.game.sounds['chest_open'].play(); self.combo_multiplier += 1.0*(2 if 'glass_cannon' in self.active_perks else 1)
                    self.combo_timer = self.game.COMBO_DURATION
                    for _ in range(50):
                        self.sparks.append([[chest_pos[0]+8, chest_pos[1]+8 + self.height], [self.random.random()*2-1, self.random.random()-2], self.random.random()*3+3, 0.01, (12,8,2), True, 0.05])
                    self.tiles[pos]['type'] = 'opened_chest'; self.player.jumps = min(self.player.jumps + 1, self.player.jumps_max)
                    self.player.attempt_jump(); self.player.velocity[1] = -3.5
                    ...
                    if self.random.randint(1, 5) < item_luck:
                        ...
                        self.items.append(Item(self.game.animation_manager, (pos[0]*self.game.TILE_SIZE+5, (pos[1]-1)*self.game.TILE_SIZE+5), (6,6), item_type, self, velocity=[self.random.random()*5-2.5, self.random.random()*2-5]))
                    else:
                        coins_mult = 2 if 'greedy' in self.active_perks else 1
                        for _ in range(self.random.randint(2,6)*coins_mult):
                            self.items.append(Item(self.game.animation_manager, (pos[0]*self.game.TILE_SIZE+5, (pos[1]-1)*self.game.TILE_SIZE+5), (6,6), 'coin', self, velocity=[self.random.random()*5-2.5, self.random.random()*2-7]))
    ...
    def update_items(self):
        ...
        for i, item in sorted(enumerate(self.items), reverse=True):
            ...
            if item.time > 30 and item.rect.colliderect(self.player.rect):
                if item.type == 'coin':
                    self.game.sounds['coin'].play(); self.coins += int(1 * self.combo_multiplier)
                    self.combo_multiplier += 0.1 * (2 if 'glass_cannon' in self.active_perks else 1); self.combo_timer = self.game.COMBO_DURATION
                    for _ in range(25):
        ...
        for i, p in sorted(enumerate(self.projectiles), reverse=True):
            ...
            for j, tile_drop in sorted(enumerate(self.tile_drops), reverse=True):
                ...
                if p.rect.colliderect(r):
                    ...
                    for _ in range(15):
                        angle, speed = self.random.random() * math.pi * 2, self.random.random() * 2
                        self.sparks.append([[tile_drop[0] + 8, tile_drop[1] + 8], [math.cos(angle) * speed, math.sin(angle) * speed], self.random.random() * 3 + 2, 0.08, (251, 245, 239), True, 0.1])
            ...
            for k, crystal_rect in sorted(enumerate(self.crystals), reverse=True):
                if p.rect.colliderect(crystal_rect):
                    self.crystals.pop(k)
                    if i < len(self.projectiles): self.projectiles.pop(i)
                    self.screen_shake = max(self.screen_shake, 7); self.game.sounds['explosion'].play()
    ...
    def update_player(self):
        if not self.dead:
            ...
            tile_pos_below_player = (self.player.rect.midbottom[0] // self.game.TILE_SIZE, self.player.rect.midbottom[1] // self.game.TILE_SIZE)
            if self.player.collisions['bottom'] and tile_pos_below_player in self.tiles:
                tile_type = self.tiles[tile_pos_below_player]['type']
                if tile_type == 'conveyor_l': self.player.velocity[0] -= 0.15 * self.time_scale
                elif tile_type == 'conveyor_r': self.player.velocity[0] += 0.15 * self.time_scale
            ...
            if collisions['bottom']:
                if tile_pos_below_player in self.tiles:
                    ...
                    if tile_type == 'fragile' and 'timer' not in self.tiles[tile_pos_below_player]['data']: self.game.sounds['block_land'].play(); self.tiles[tile_pos_below_player]['data']['timer'] = 90
                    elif tile_type == 'geyser' and self.tiles[tile_pos_below_player].get('data', {}).get('timer', 0) <= 0: self.tiles[tile_pos_below_player]['data']['timer'] = 120
                        ...
                        for _ in range(4):
                            angle = self.random.random() * math.pi * 2; speed = self.random.random() * 1.5
            ...
            for pos, data in self.tiles.items():
                if data.get('type') == 'geyser' and data.get('data', {}).get('timer', 0) > 0:
                    ...
                    if int(data['data']['timer']) == 90:
                        self.game.sounds['super_jump'].play(); self.screen_shake = 10
                        for _ in range(40):
                            angle, speed = self.random.uniform(math.pi * 1.2, math.pi * 1.8), self.random.uniform(1.5, 3.5)
                            self.sparks.append([[pos[0]*16 + 8, pos[1]*16], [math.cos(angle) * speed, math.sin(angle) * speed * 2], self.random.random() * 3 + 2, 0.04, (200, 200, 255), True, 0.1])
        ...
        if self.tiles:
            try:
                ...
                for y in range(highest_base_row, 0, -1):
                    if all((i + 1, y) in self.tiles for i in range(self.game.WINDOW_TILE_SIZE[0] - 2)):
    ...
    def use_item(self):
        if 'butter_fingers' in self.active_curses and self.random.random() < 0.5:
            self.game.sounds['combo_end'].play(); self.current_item = None; return
        self.item_used = True; item = self.current_item
        if item == 'warp':
        ...
        elif item == 'jump': self.game.sounds['super_jump'].play(); self.screen_shake = 12; self.player.jumps = self.player.jumps_max + 1; self.player.attempt_jump(); self.player.velocity[1] = -8 
        elif item == 'cube':
            self.game.sounds['block_land'].play(); place_pos_x = int(self.player.center[0] // self.game.TILE_SIZE); place_pos_y = int(self.player.pos[1] // self.game.TILE_SIZE) + 1
            base_row = (max(self.tiles, key=lambda x: x[1])[1] if self.tiles else self.game.WINDOW_TILE_SIZE[1]-1)
            for i in range(place_pos_y, base_row + 1): self.tiles[(place_pos_x, i)] = {'type': 'placed_tile', 'data': {}}; self.recalculate_stack_heights()
        elif item == 'bomb':
            if 'explosion' in self.game.sounds: self.game.sounds['explosion'].play()
            ...
            for pos in to_bomb:
                if tuple(pos) in self.tiles:
                    for _ in range(15):
                        angle, speed = self.random.random() * math.pi * 2, self.random.random() * 2
                        self.sparks.append([[pos[0]*self.game.TILE_SIZE+8, pos[1]*self.game.TILE_SIZE+8], [math.cos(angle)*speed, math.sin(angle)*speed], self.random.random()*4+2, 0.08, (12,2,2), True, 0.1])
    ...
    def render_lava(self, surf):
        if self.lava_y < self.game.DISPLAY_SIZE[1]:
            ...
            color = (255, 150, 40) if self.master_clock % 20 < 10 else (255, 60, 20)
            pygame.draw.rect(surf, color, lava_rect)
            if self.master_clock % 4 == 0:
                pos = [self.random.randint(0, self.game.DISPLAY_SIZE[0]), self.lava_y + 5]
    ...
    def render_crystals(self, surf):
        for crystal_rect in self.crystals:
            ...
            if -32 < render_pos[1] < self.game.DISPLAY_SIZE[1]: surf.blit(self.crystal_img, render_pos)
    ...
    def render_placed_tiles(self, surf):
        for pos, data in self.tiles.items():
            ...
            surf.blit(img_map.get(data.get('type'), self.tile_img), blit_pos)
            if data['type'] in ['tile', 'placed_tile'] and (pos[0], pos[1]-1) not in self.tiles: surf.blit(self.tile_top_img, blit_pos)
            if data.get('type') == 'chest': surf.blit(self.chest_img, (blit_pos[0], blit_pos[1] - self.game.TILE_SIZE))
            ...
            if data.get('type') == 'conduit' and self.master_clock % 4 == 0:
                self.sparks.append([[blit_pos[0] + self.random.random() * 16, blit_pos[1] + self.random.random() * 16], [0, 0], self.random.random() * 1.5, 0.1, (150, 180, 255), False, 0])
    ...
    def render_sparks(self, surf):
        for spark in self.sparks:
            size, pos = int(spark[2]), (spark[0][0], spark[0][1] - self.height)
            if size > 0:
    ...
    def render_borders(self, surf):
        ...
        border_to_blit, light_border_to_blit = self.border_img.copy(), self.border_img_light.copy()
        if self.mode == "hardcore":
            border_to_blit.fill((255,50,50, 200), special_flags=pygame.BLEND_RGBA_MULT)
            light_border_to_blit.fill((255,80,80, 150), special_flags=pygame.BLEND_RGBA_MULT)
        surf.blit(light_border_to_blit, (0, -math.sin(self.master_clock/30)*4-7)); surf.blit(border_to_blit, (0, -math.sin(self.master_clock/40)*7-14))
        surf.blit(pygame.transform.flip(light_border_to_blit, 0, 1), (0, self.game.DISPLAY_SIZE[1]+math.sin(self.master_clock/40)*3+9-self.border_img.get_height()))
        surf.blit(pygame.transform.flip(border_to_blit, 0, 1), (0, self.game.DISPLAY_SIZE[1]+math.sin(self.master_clock/30)*3+16-self.border_img.get_height()))
    ...
    def render_hud(self, surface):
        ...
        self.game.black_font.render(str(self.coins), surface, (23, 11)); self.game.white_font.render(str(self.coins), surface, (22, 10))
        time_meter_center, time_meter_radius = (66, 16), 7
        ...
        if time_meter_pct > 0:
            start_angle, end_angle = math.pi/2, start_angle-(2*math.pi*time_meter_pct)
        ...
        if self.combo_multiplier > 1.0:
            ...
            self.game.black_font.render(text, surface, (x+1, y+1), scale=2); combo_font.render(text, surface, (x, y), scale=2)
            bar_w = (self.combo_timer / self.game.COMBO_DURATION) * 60; bar_x = self.game.DISPLAY_SIZE[0]//2 - 30
            pygame.draw.rect(surface, (0, 0, 1), [bar_x, 21, 60, 4]); pygame.draw.rect(surface, (251, 245, 239), [bar_x, 20, bar_w, 4])
        ...
        if self.current_item:
            if (self.master_clock % 50 < 12) or (abs(self.master_clock % 50-20) < 3): surface.blit(self.item_slot_flash_img, item_slot_pos)
            surface.blit(self.game.item_icons[self.current_item], (item_slot_pos[0]+5, item_slot_pos[1]+5))
        prompt_font = Font(self.game.get_path('data','fonts','small_font.png'), (255,230,90)); prompt_font.render("E/X", surface, (item_slot_pos[0]-16, item_slot_pos[1]+4))
        bar_x, bar_y, bar_w, bar_h = top_right_panel_rect.left+5, top_right_panel_rect.bottom-12, top_right_panel_rect.width-10, 6
        ...
        fill_w = int((bar_w-2)*(self.player.focus_meter/self.player.FOCUS_METER_MAX)); pygame.draw.rect(surface, color, [bar_x+1, bar_y+1, fill_w, bar_h-2])
        if self.prophecy:
            prop_y, prop_text = self.game.DISPLAY_SIZE[1] - 32, self.prophecy['flavor_text']
            if self.prophecy.get('completed'):
                prop_text = "Prophecy Fulfilled!"; color = (255,230,90) if self.master_clock%20>10 else (255,255,255)
                prop_font = Font(self.game.get_path('data', 'fonts', 'small_font.png'), color); w = prop_font.width(prop_text)
                prop_font.render(prop_text, surface, (self.game.DISPLAY_SIZE[0]//2-w//2, prop_y))
            else:
                progress = f"({int(self.prophecy_progress)}/{self.prophecy['value']})"; w_text, w_prog = self.game.white_font.width(prop_text), self.game.white_font.width(progress)
        ...
        if self.active_perks:
            num_perks = len(self.active_perks); perk_panel_w = 6 + (num_perks*12); perk_panel_rect = pygame.Rect(4, self.game.DISPLAY_SIZE[1]-22, perk_panel_w, 18)
            render_panel_9slice(surface, perk_panel_rect, self.panel_img, 8); x_off = perk_panel_rect.left + 5
            for key in sorted(list(self.active_perks)):
                if key in self.game.perk_icons:
                    icon = self.game.perk_icons[key]; surface.blit(icon, (x_off, perk_panel_rect.top + 4)); x_off += icon.get_width() + 2