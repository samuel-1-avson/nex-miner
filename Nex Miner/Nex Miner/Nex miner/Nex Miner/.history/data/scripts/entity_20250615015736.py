...
class Entity:
    ...
    def set_action(self, action_id, force=False):
        # --- REFINED to handle dynamic character sprites, including empty/invalid ones ---
        animation_name = self.type + '_' + action_id
        
        # Check if a character-specific animation exists AND has frames.
        # If it exists but is empty (like the placeholder ninja animations), it's considered invalid.
        is_valid_specific_anim = False
        if animation_name in self.assets.animations:
            # Check the animation data to see if it has any actual frames.
            if self.assets.animations[animation_name].frame_surfs:
                is_valid_specific_anim = True
                
        # If the specific animation is not found or is invalid, fall back to the default 'player' animation.
        if not is_valid_specific_anim:
            # This check is a bit of a hack, but works for this project structure.
            if 'player' in self.type or self.type == 'ninja': # Simple check for player-like entities
                 animation_name = 'player' + '_' + action_id

        # Proceed with the original logic using the correctly determined animation name.
