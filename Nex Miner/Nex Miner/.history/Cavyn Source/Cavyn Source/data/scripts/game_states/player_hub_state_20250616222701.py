...
class PlayerHubState(State):
    def __init__(self, game):
        super().__init__(game)
        self.tabs = ["STATS", "COMPENDIUM", "ORACLE"] # --- NEW ---
        self.current_tab = 0
    ...
    def update(self):
        ...
        if not self.game.ai_agent.is_thinking:
            self.ai_input_active = True

    def ask_oracle(self):
        if self.ai_input_text and not self.game.ai_agent.is_thinking:
            self.ai_input_active = False # Deactivate input while waiting
            # --- NEW: Create a more contextual prompt for the AI ---
    ...
    def render(self, surface):
        ...
        elif self.tabs[self.current_tab] == "ORACLE": self.render_oracle(surface, p_rect, content_y) # --- NEW ---
    ...
    def render_oracle(self, surface, p_rect, y):
        ...
        response_box = pygame.Rect(p_rect.left + 10, y, p_rect.width - 20, 90)
        if self.game.ai_agent.is_thinking:
            ...
            self.game.white_font.render(self.game.ai_agent.response, surface, (response_box.x, response_box.y), line_width=response_box.width)
        else:
            self.game.white_font.render("Ask the Oracle a question.", surface, (response_box.x, response_box.y), line_width=response_box.width)
