...
class GameplayState(State):
    ...
    def reset(self):
        ...
        # --- NEW: Apply Character and Artifact Mods ---
        ...
        # Clear the prophecy from the save file so it's only for this run
        ...
        if self.mode == "zen":
            ...
            self.active_curses = {random.choice(list(self.game.curses.keys()))} if list(self.game.curses.keys()) else set()
        else:
    ...
    def render(self, surface):
        ...
        if self.invincibility_timer > 0 and self.master_clock % 6 < 3:
            # Flashing effect by skipping render
            pass
        else:
    ...
    def handle_death(self):
        ...
        if self.mode == "zen":
            self.player.pos[1] -= 20; self.player.velocity[1] = -5; return
        if self.player.pos[1] + self.player.size[1] > self.lava_y and not self.dead:
            self.dead = True; self.game.sounds['death'].play(); self.screen_shake = 20
            background_surf = self.game.display.copy(); self.render(background_surf); self.game.push_state(GameOverState(self.game, self.coins, background_surf)); return
        if self.player_shielded:
            self.player_shielded = False; self.game.sounds['explosion'].play() if 'explosion' in self.game.sounds else self.game.sounds['super_jump'].play(); self.screen_shake = 15
        ...
        background_surf = self.game.display.copy(); self.render(background_surf); self.game.push_state(GameOverState(self.game, self.coins, background_surf))
    ...
    def update_special_entities(self):
        ...
        for i, crystal_rect in sorted(enumerate(self.crystals), reverse=True):
            if self.player.dash_timer > 0 and self.player.rect.colliderect(crystal_rect):
                self.crystals.pop(i)
                self.coins += 5
                ...
                for k in range(20):
                    ...
                    self.sparks.append([[crystal_rect.centerx, crystal_rect.centery], [math.cos(angle) * speed, math.sin(angle) * speed], random.uniform(2, 5), 0.08, (200, 220, 255), True, 0.1])
                continue

            if self.freeze_timer <= 0: crystal_rect.y += 1.6 * self.time_scale
            if self.player.rect.colliderect(crystal_rect): self.crystals.pop(i); self.handle_death()
    ...
    def update_prophecy(self):
        ...
        if self.prophecy['objective_type'] == 'collect_coins':
            self.prophecy_progress = self.coins
        elif self.prophecy['objective_type'] == 'reach_combo':
            self.prophecy_progress = self.combo_multiplier
        ...
        if self.prophecy_progress >= self.prophecy['value']:
            ...
            # Grant reward
            if self.prophecy['reward_type'] == 'coins':
                self.coins += self.prophecy['reward_value']
            elif self.prophecy['reward_type'] == 'item':
                self.current_item = self.prophecy['reward_value']
        ...
        for i, tile in sorted(enumerate(self.tile_drops), reverse=True):
            ...
            if self.player.dash_timer > 0 and r.colliderect(self.player.rect):
                ...
                if 'brittle_blocks' not in self.active_curses:
                    self.coins += 2
                    if self.prophecy and self.prophecy['objective_type'] == 'collect_coins':
                        self.prophecy_progress += 2
                
                # --- NEW: Prophecy Tracking ---
                if self.prophecy and self.prophecy['objective_type'] == 'destroy_tiles_dash':
                    self.prophecy_progress += 1
    ...
    def update_placed_tiles(self):
        ...
        for tile_pos, tile_data in self.tiles.items():
            player_on_tile = self.player.rect.colliderect(pygame.Rect(tile_pos[0]*16, tile_pos[1]*16-2, 16, 18))
