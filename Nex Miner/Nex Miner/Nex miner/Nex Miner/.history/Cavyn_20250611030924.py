...
spike_tile_img = load_img('data/images/spike_tile.png') # <-- NEW
...
while True:
    ...
    if master_clock > 180:
        ...
        if game_timer > 10 + 25 * (20000 - min(20000, master_clock)) / 20000:
            ...
            # <-- MODIFIED: Add spawning logic for spike blocks
            roll = random.randint(1, 25) # Increased range to make spikes rarer
            if roll == 1:
                tile_type = 'chest'
            elif roll < 4:
                tile_type = 'fragile'
                ...
                tile_type = 'bounce'
            elif roll == 7: # <-- NEW: Spike blocks are rare
                tile_type = 'spike'
            else:
                tile_type = 'tile'
    ...
    for tile_pos in tiles:
        ...
        if tile_type == 'spike': # <-- NEW: Render spike tile
    ...
    if not dead:
        ...
        if collisions['bottom']:
            ...
            if tile_pos_below in tiles:
                ...
                elif tile_type_below == 'spike': # <-- NEW: Spike death logic
                    sounds['death'].play()
                    player.velocity = [0, -4] # Pop the player up a bit
                    for i in range(380):
                        angle = random.random() * math.pi
                        speed = random.random() * 1.5
                        physics_on = random.choice([False, True, True])
