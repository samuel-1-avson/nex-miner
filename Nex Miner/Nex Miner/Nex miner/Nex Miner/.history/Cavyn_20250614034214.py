...
class Player(Entity):
    def __init__(self, *args):
        ...
        self.DASH_SPEED = 7 # Slightly reduced for better control
        self.DASH_DURATION = 9 # frames
    ...
    def attempt_dash(self):
        # Can dash if cooldown is over AND you either have an air-dash or are on the ground
        if self.dash_cooldown <= 0 and (self.has_air_dash or self.air_time < 6):
            if self.air_time >= 6: # Consumed air dash
                self.has_air_dash = False
                
            self.dash_timer = self.DASH_DURATION
            self.dash_cooldown = self.DASH_COOLDOWN # Start cooldown immediately
            sounds['warp'].play()

            direction = 1 if self.flip[0] else -1
            for i in range(25):
                angle = random.uniform(-math.pi / 2.5, math.pi / 2.5) # Cone shape
                vel_x = math.cos(angle + (math.pi if direction == 1 else 0))
                vel_y = math.sin(angle)
                speed = random.uniform(1.0, 3.5)
                sparks.append([self.center.copy(), [vel_x * speed, vel_y * speed], random.uniform(2, 5), 0.08, (200, 220, 255), True, 0.05])
    ...
    def update(self, tiles, time_scale=1.0):
        ...
        if self.dash_cooldown > 0: self.dash_cooldown -= 1 * time_scale
        if self.dash_timer > 0: self.dash_timer -= 1 * time_scale
        
        # === HORIZONTAL MOVEMENT ===
        if self.dash_timer > 0:
            # Player is dashing: Force velocity and create particles.
            direction = 1 if self.flip[0] else -1
            self.velocity[0] = direction * self.DASH_SPEED
            ...
            self.velocity[0] += (target_velocity_x - self.velocity[0]) * 0.3 # Increased interpolation for snappier control
            if abs(self.velocity[0]) < 0.1:
                self.velocity[0] = 0

        # === VERTICAL MOVEMENT (GRAVITY) ===
        if self.dash_timer > 0:
            # No gravity during the dash for a clean horizontal line.
            self.velocity[1] = 0
        else:
            self.velocity[1] = min(self.velocity[1] + 0.3, 4)

        # === ANIMATION AND VISUALS ===
        # Set facing direction based on movement, but not while dashing
        ...
        # Move the player based on the final calculated velocity
        collisions = self.move(self.velocity, tiles, time_scale)
        ...
        if collisions['bottom']:
            ...
            self.air_time = 0
            self.jumps = self.jumps_max
            self.has_air_dash = True # Refresh air dash on ground
            self.jumping = False
            self.rotation = 0
            ...
            if was_on_ground:
                # Started falling: start coyote timer
                self.coyote_timer = 6
