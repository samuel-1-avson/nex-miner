...
import random
import math # Import math for wall slide particles
...
class Player(Entity):
    def __init__(self, assets, pos, size, type, state):
        ...
        # --- NEW: Wall slide attribute ---
        self.is_wall_sliding = False

    def attempt_jump(self):
        # --- NEW: Ghost/After-image effect ---
        # Add a ghost to the gameplay state's list when jumping.
        # Format: [image, position, timer, flip]
        ghost_img = self.img.copy()
        ghost_img.set_alpha(120)
        self.state.ghosts.append([ghost_img, self.pos.copy(), 15, self.flip[0]])

        # --- MODIFIED: Wall Jump Logic ---
        jump_velocity = -5
        # If sliding on a wall, perform a stronger wall jump
        ...
        if self.jumps > 0 or self.coyote_timer > 0:
            if self.jumps == self.jumps_max or self.coyote_timer > 0:
                ...
                self.velocity[1] = -4 # Standard double jump
                for i in range(24):
    ...
    def update(self, tiles, time_scale=1.0):
        ...
        # --- MODIFIED: Gravity and Wall Slide ---
        # Assume not wall sliding until conditions are met
        self.is_wall_sliding = False
        if self.dash_timer > 0:
            self.velocity[1] = 0
        else:
            ...
            if self.air_time > 2 and (self.collisions['left'] or self.collisions['right']):
                # To slide, player must be holding the direction towards the wall
                if (self.left and self.collisions['left']) or (self.right and self.collisions['right']):
                    self.is_wall_sliding = True
                    self.velocity[1] = min(self.velocity[1], 1.2) # Slower fall speed
                    # Spawn scraping particles
        ...
        # --- MODIFIED: Animation logic for Wall Slide ---
        if self.is_charging_dash: self.set_action('idle')
        elif self.dash_timer > 0: self.set_action('jump')
        elif self.is_wall_sliding: self.set_action('jump') # Use jump sprite for wall slide
        ...
        was_in_air_for_long = self.air_time > 5 # Threshold for landing poof
        ...
        if self.collisions['bottom']:
            self.velocity[1] = 0
            # --- NEW: Landing Poof and Sound ---
            if was_in_air_for_long:
                self.state.game.sounds['land'].play()
                for i in range(10):
                    angle = random.uniform(math.pi * 0.9, math.pi * 2.1)
                    speed = random.uniform(0.5, 1.5)
                    self.state.sparks.append([self.rect.midbottom, [math.cos(angle) * speed, -math.sin(angle) * speed], random.uniform(2, 4), 0.1, (180, 180, 190), True, 0.02])
