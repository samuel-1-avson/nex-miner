...
class GameplayState(State):
    ...
    def reset(self):
        ...
        # Ensure all upgrades have a default value in save_data
        ...
        if self.game.save_data['upgrades'].get('starting_item', 0) > 0:
            possible_items = ['cube', 'jump', 'bomb', 'shield', 'freeze']
    ...
    def update(self):
        ...
        if not self.dead:
            self.update_biome(); self.update_combo_meter(); self.update_perk_offering()
    ...
    def update_biome(self):
        ...
        if self.mode != "zen" and self.current_biome_index + 1 < len(self.game.biomes) and self.coins >= self.game.biomes[self.current_biome_index + 1]['score_req']: next_biome_index += 1
        if next_biome_index != self.current_biome_index:
            self.current_biome_index = next_biome_index; self.game.load_biome_bgs(self.game.biomes[self.current_biome_index])
    ...
    # (The rest of the file from update_time_scale onward remains the same as the previous full version)
    # ... Omitted for brevity ...
