...
class GameplayState(State):
    ...
    def update_biome(self):
        ...
        if next_biome_index != self.current_biome_index:
            ...
            self.game.load_ biome_bgs(self.game.biomes[self.current_biome_index])
        ...
        if not self.dead:
            ...
            if collisions['bottom']:
                ...
                if tile_pos_below in self.tiles:
                    ...
                    elif tile_type == 'geyser' and self.tiles[tile_pos_below]['data'].get('timer', 0) <= 0: self.tiles[tile_pos_below]['data']['timer'] = 120 # Cooldown
    ...
    def fire_projectile(self, keys):
        tile_pos_below = (self.player.rect.midbottom[0] // self.game.TILE_SIZE, self.player.rect.midbottom[1] // self.game.TILE_SIZE)
        player_on_prism = self.player.collisions['bottom'] and tile_pos_below in self.tiles and self.tiles[tile_pos_below]['type'] == 'prism'
    ...
    def render_crystals(self, surf):
        for crystal_rect in self.crystals:
            ...
            if render_pos[1] > -32 and render_pos[1] < self.game.DISPLAY_SIZE[1]:
    ...
    def render_placed_tiles(self, surf):
        for pos, data in self.tiles.items():
            ...
            img_map = {
                'placed_tile': self.placed_tile_img, 'greed': self.greed_tile_img, 
                'magnetic': self.magnetic_tile_img, 'unstable': self.unstable_tile_img,
                'sticky': self.sticky_tile_img, 'fragile': self.fragile_tile_img,
                'bounce': self.bounce_tile_img, 'spike': self.spike_tile_img,
                'prism': self.prism_tile_img, 'geyser': self.geyser_tile_img,
            }
    ...
    def render_falling_tiles(self, surf):
        for tile in self.tile_drops:
            ...
            img_map = {
                'greed': self.greed_tile_img, 'motherlode': self.motherlode_tile_img,
                'unstable': self.unstable_tile_img, 'magnetic': self.magnetic_tile_img,
                'sticky': self.sticky_tile_img, 'fragile': self.fragile_tile_img,
                'bounce': self.bounce_tile_img, 'spike': self.spike_tile_img,
                'prism': self.prism_tile_img, 'geyser': self.geyser_tile_img,
            }
