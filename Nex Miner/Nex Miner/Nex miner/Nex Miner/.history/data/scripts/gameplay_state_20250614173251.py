...
class GameplayState(State):
            ...
            if self.player.dash_timer > 0 and r.colliderect(self.player.rect):
                ...
                if 'overcharge' in self.active_perks:
                    ...
                    for k_inner, other_tile_data in enumerate(self.tile_drops):
                        ...
                        if i != k_inner and r.inflate(self.game.TILE_SIZE, self.game.TILE_SIZE).colliderect(other_tile_rect):
        ...
        for pos, data in list(self.tiles.items()):
            if data['type'] == 'chest':
                ...
                if r.colliderect(self.player.rect):
                    ...
                    self.combo_timer = self.game.COMBO_DURATION
                    for _ in range(50):
                        self.sparks.append([[chest_pos[0]+8, chest_pos[1]+8], [random.random()*2-1, random.random()-2], random.random()*3+3, 0.01, (12,8,2), True, 0.05])
                    self.tiles[pos]['type'] = 'opened_chest'
                    self.player.jumps += 1; self.player.attempt_jump(); self.player.velocity[1] = -3.5
                    item_luck = 3 + self.game.save_data['upgrades']['item_luck']
                    if random.randint(1, 5) < item_luck:
                        self.items.append(Item(self.game.animation_manager, (pos[0]*self.game.TILE_SIZE+5, (pos[1]-1)*self.game.TILE_SIZE+5), (6,6), random.choice(['warp', 'cube', 'jump', 'bomb', 'freeze', 'shield']), self, velocity=[random.random()*5-2.5, random.random()*2-5]))
                    else:
                        coins_mult = 2 if 'greedy' in self.active_perks else 1
    ...
    def update_items(self):
        ...
        for i, item in sorted(enumerate(self.items), reverse=True):
            ...
            item.update(edge_rects, self.time_scale)
            ...
            if item.type == 'coin' and not self.dead:
                ...
                if magnet_lvl > 0:
                    ...
                    dist = math.sqrt(dist_x**2 + dist_y**2) if (dist_x**2 + dist_y**2) > 0 else 1
    ...
    def render_placed_tiles(self, surf):
        for pos, data in self.tiles.items():
            ...
            img_map = {'placed_tile': self.placed_tile_img, 'greed': self.greed_tile_img, 'magnetic': self.magnetic_tile_img,
                       ...
                       'bounce': self.bounce_tile_img, 'spike': self.spike_tile_img, 'chest': self.chest_img, 'opened_chest': self.opened_chest_img}
            if data['type'] in img_map:
                if data['type'] in ['chest', 'opened_chest']:
                    blit_pos = (pos[0]*self.game.TILE_SIZE, (pos[1]-1)*self.game.TILE_SIZE + int(self.height))
                surf.blit(img_map[data['type']], blit_pos)
            else: # Default 'tile'
                surf.blit(self.tile_img, blit_pos)
            ...
            if data['type'] == 'chest' and random.randint(1, 20) == 1:
                chest_pos = (pos[0]*self.game.TILE_SIZE, (pos[1]-1)*self.game.TILE_SIZE)
                self.sparks.append([[chest_pos[0]+2+12*random.random(), chest_pos[1]+4+8*random.random()], [0, random.random()*0.25-0.5], random.random()*4+2, 0.023, (12,8,2), True, 0.002])

    def render_falling_tiles(self, surf):
        for tile in self.tile_drops:
            pos = (tile[0], tile[1] + self.height)
            img_map = {'greed': self.greed_tile_img, 'motherlode': self.motherlode_tile_img, 'unstable': self.unstable_tile_img, 'magnetic': self.magnetic_tile_img, 'sticky': self.sticky_tile_img, 'fragile': self.fragile_tile_img, 'bounce': self.bounce_tile_img, 'spike': self.spike_tile_img}
    ...
    def render_sparks(self, surf):
        for spark in self.sparks:
            ...
            surf.blit(glow_img(int(size * 1.5 + 2), (int(spark[4][0]/2),int(spark[4][1]/2),int(spark[4][2]/2))), (pos[0]-size*2, pos[1]+self.height-size*2), special_flags=BLEND_RGBA_ADD)
            surf.blit(glow_img(size, spark[4]), (pos[0]-size, pos[1]+self.height-size), special_flags=BLEND_RGBA_ADD)
