...
fragile_tile_img = load_img('data/images/fragile_tile.png') # <-- NEW
...
class Player(Entity):
    ...
    def update(self, tiles):
        ...
        return collisions # <-- MODIFIED: return collisions for use in main loop
...
for i in range(WINDOW_TILE_SIZE[0] - 2):
    # <-- MODIFIED: Use new dictionary structure for tiles
...
while True:
    ...
    if master_clock > 180:
        ...
        if game_timer > 10 + 25 * (20000 - min(20000, master_clock)) / 20000:
            ...
            # <-- MODIFIED: Add spawning logic for fragile blocks
    ...
    for i, tile in sorted(enumerate(tile_drops), reverse=True):
        ...
        if check_pos in tiles:
            ...
            # <-- MODIFIED: Place a dictionary instead of a string
            tiles[place_pos] = {'type': tile[2], 'data': {}}
            sounds['block_land'].play()
            # <-- MODIFIED: Access the type property of the tile dictionary
            if tiles[check_pos]['type'] == 'chest':
                tiles[check_pos]['type'] = 'tile' # Convert chest to a normal tile
    ...
    for tile_pos in tiles:
        # <-- MODIFIED: Generate rects from dictionary keys for collision
        tile_rects.append(pygame.Rect(TILE_SIZE * tile_pos[0], TILE_SIZE * tile_pos[1], TILE_SIZE, TILE_SIZE))

        # <-- MODIFIED: Render loop based on tile type
        ...
        if tile_type == 'chest':
            ...
            if chest_r.colliderect(player.rect):
                ...
                tiles[tile_pos]['type'] = 'opened_chest' # <-- MODIFIED: Change type
                player.jumps += 1
                player.attempt_jump()
    ...
    if height != target_height:
        ...
        if abs(target_height - height) < 0.2:
            ...
            for i in range(WINDOW_TILE_SIZE[0] - 2):
                if (i + 1, base_row + 1) in tiles: # Check before deleting
    ...
    edge_rects = [] # <-- MODIFIED: Renamed from tile_rects to avoid confusion
    ...
    for i, item in sorted(enumerate(items), reverse=True):
        ...
        if lookup_pos in tiles:
            items.pop(i)
            continue
        item.update(edge_rects + lookup_nearby(tiles, item.center)) # <-- MODIFIED
        if item.time > 30:
    ...
    if not dead:
        # <-- MODIFIED: Get collision results from player update
        collisions = player.update(tile_drop_rects + edge_rects + tile_rects)

        # <-- NEW: Logic to activate fragile tiles on landing
        if collisions['bottom']:
            player_feet_pos = player.rect.midbottom
            # Check the tile exactly at the player's feet
            tile_pos_below = (player_feet_pos[0] // TILE_SIZE, player_feet_pos[1] // TILE_SIZE)
            
            if tile_pos_below in tiles and tiles[tile_pos_below]['type'] == 'fragile':
                # Activate the timer if it's not already active
                if 'timer' not in tiles[tile_pos_below]['data']:
                    sounds['block_land'].play() # Re-use the land sound for a cracking noise
                    tiles[tile_pos_below]['data']['timer'] = 90 # 1.5 seconds at 60 FPS
    else:
        player.opacity = 80
        player.update([])
        player.rotation -= 16
        ...
        if event.type == KEYDOWN:
                ...
                if not dead:
                    ...
                    if current_item == 'cube':
                        ...
                        for i in range(place_pos[1], base_row + 2):
                             # <-- MODIFIED: Place a dictionary when using cube item
            ...
            if event.key == K_r:
                if dead:
                    ...
                    for i in range(WINDOW_TILE_SIZE[0] - 2):
                        # <-- MODIFIED: Reset with new dictionary structure
                        tiles[(i + 1, WINDOW_TILE_SIZE[1] - 1)] = {'type': 'tile', 'data': {}}
