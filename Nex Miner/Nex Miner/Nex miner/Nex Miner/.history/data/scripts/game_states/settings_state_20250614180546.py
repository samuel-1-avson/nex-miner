...
class SettingsState(State):
    ...
    def render(self, surface):
        ...
        for i, key in enumerate(self.options_keys):
            ...
            color = self.highlight_color if i == self.selection_index else self.game.white_font.color
            font = self.game.black_font if i == self.selection_index else self.game.white_font
            
            name_pos = (p_rect.left + 15, y_pos)
            if i == self.selection_index:
                 self.game.white_font.render(name_text, surface, (name_pos[0]-1, name_pos[1]-1), color=color) # Simple glow
            
            font.render(name_text, surface, name_pos, color=color)
    ...
    def render_option_editor(self, surface, key, p_rect, y_pos):
        ...
        if key.endswith("_volume"): # Draw a percentage bar
            ...
            color = (120, 220, 130) if value else (200, 80, 80)
            self.game.white_font.render(f"[{text}]", surface, (editor_x, y_pos), color=color)