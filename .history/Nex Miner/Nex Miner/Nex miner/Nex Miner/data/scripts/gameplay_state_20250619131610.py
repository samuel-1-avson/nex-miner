...
class GameplayState(State):
    ...
    def render_hud(self, surface):
        # --- NEW: Minimal Top-Left HUD (Coins & Time) ---
        x_offset = 6
        ...
        x_offset += self.game.white_font.width(str(self.coins)) + 12

        time_meter_center = (x_offset + 7, 12)
        time_meter_radius = 7
        time_meter_pct = self.time_meter / self.game.time_meter_max
        
        pygame.draw.circle(surface, (10, 20, 40), time_meter_center, time_meter_radius + 1, 2)
        if time_meter_pct > 0:
            ...
            arc_rect = pygame.Rect(time_meter_center[0] - time_meter_radius, time_meter_center[1] - time_meter_radius, time_meter_radius*2, time_meter_radius*2)
            pygame.draw.arc(surface, color, arc_rect, end_angle, start_angle, 2)
        ...
        # --- NEW: Minimal Top-Right HUD (Item & Focus Ring) ---
        ...
        if self.player.is_charging_dash:
            ...
            focus_color = (255, 150, 40) if self.player.focus_meter >= self.player.FOCUS_DASH_COST else (100, 60, 20)
        
        pygame.draw.circle(surface, (10, 20, 40, 180), item_slot_center, focus_radius, 2)
        ...
        if focus_pct > 0:
            ...
            pygame.draw.arc(surface, focus_color, arc_rect, end_angle, start_angle, 2)
        
        item_icon_base_pos = (item_slot_center[0] - self.item_slot_img.get_width() // 2, item_slot_center[1] - self.item_slot_img.get_height() // 2)
        surface.blit(self.item_slot_img, item_icon_base_pos)

        if self.current_item:
            if (self.master_clock % 50 < 12) or (abs(self.master_clock % 50 - 20) < 3):
                surface.blit(self.item_slot_flash_img, item_icon_base_pos)
        ...
        # --- Directive (Position Modified) ---
        if self.directive:
            prop_y = self.game.DISPLAY_SIZE[1] - 20 # Lowered position
