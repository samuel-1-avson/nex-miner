...
class GameplayState(State):
    ...
    def reset(self):
        ...
        if self.mode == "zen":
            self.spawn_timer_base = 80  # Slower tile spawns
            self.disabled_tiles = {'spike', 'unstable'}
            self.active_curses = set()
            self.combo_multiplier = 1.0
            # Quiet the default music a bit for a zen feel
            pygame.mixer.music.set_volume(0.3 * self.game.save_data['settings'].get('music_volume', 1.0))
        elif self.mode == "hardcore":
            self.spawn_timer_base = 25  # Faster tile spawns from the start
            self.disabled_tiles = set()
            # Start with a random curse if any are available
    ...
    def handle_death(self):
        # In zen mode, you don't die.
        if self.mode == "zen":
            self.player.pos[1] -= 20 # Simple teleport up
    ...
    def update_tile_spawning(self):
        ...
        # Use the configurable spawn timer, which gets faster over time
        ...
        if self.game_timer > current_spawn_rate:
            ...
            if elite_roll <= 2:
                ...
                # Filter out tiles disabled by the current mode
                available_choices = {
                    k: v for k, v in spawn_weights.items() 
                    if k in biome['available_tiles'] and k not in self.disabled_tiles
                }
