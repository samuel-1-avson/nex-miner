...
class GameplayState(State):
    ...
    def reset(self):
        ...
        if 'starting_item' in character_data['mods']:
            ...
            # --- MODIFIED: Handle the 'random' starting item for Technomancer ---
            if item == 'random':
        ...
        if self.directive:
            ...
            self.directive = json.loads(json.dumps(self.directive)) # Deep copy
            # if 'fortune_finder' in self.active_perks: # This perk does not exist in provided files
            #     self.directive['value'] = int(self.directive['value'] * 0.75)
        ...
        if self.mode == "daily_challenge":
             ...
             # Specific logic for daily challenge could go here if needed (e.g., set start biome)
    ...
    def check_challenge_conditions(self):
        ...
        if lose_con:
            if lose_con['type'] == 'max_place' and len(self.tile_drops) > lose_con['value']:
                self.handle_death()
                return # Stop checking if a lose condition is met
        
        # Check win conditions
        if win_con['type'] == 'reach_y':
            # Check for a specific tile marked "goal" in the layout
            goal_pos = None
    ...
    def handle_challenge_win(self):
        if not self.dead: # Prevent triggering multiple times
            self.dead = True # Use 'dead' flag to stop gameplay logic
            if self.game.sounds.get('upgrade'): self.game.sounds['upgrade'].play()
            # In a full game, you'd push a "Challenge Complete" state.
            # For now, just pop back to the menu.
            self.game.pop_state()
    ...
    def render(self, surface):
        ...
        self.render_challenge_goal(surface) # Render challenge elements
        ...
        if self.mode == 'challenge':
            for pos, tile_type in self.challenge_config['start_layout']:
                if tile_type == 'goal':
                    ...
                    goal_icon = self.game.item_icons['jump'] # Use an icon as the goal marker
    ...
    def handle_death(self):
        ...
        if self.mode == 'challenge':
            self.game.pop_state() # Just go back to challenge select
        else:
    ...
    def update_ambient_hazards(self):
        if 'ambient_hazard' in self.game.biomes[self.current_biome_index]:
            ...
            if hazard['type'] == 'rising_plasma': self.plasma_y -= hazard['speed'] * self.time_scale
            if self.player.pos[1] > self.plasma_y: self.handle_death()
    ...
    def update_special_entities(self):
        ...
        for i, data_node_rect in sorted(enumerate(self.data_nodes), reverse=True):
            ...
            if self.freeze_timer <= 0: data_node_rect.y += 1.6 * self.time_scale
            ...
            if check_pos_real_world in self.tiles:
                ...
                for k in range(self.random.randint(4, 8)):
                    self.items.append(Item(self.game.animation_manager, (data_node_rect.centerx, data_node_rect.bottom), (6, 6), 'coin', self, velocity=[self.random.random() * 4 - 2, self.random.random() * 2 - 6]))
    ...
    def update_directive(self):
        if not self.directive or self.directive.get('completed'):
            return
        
        # Update progress based on objective type
        if self.directive['objective_type'] == 'collect_coins': self.directive_progress = self.coins
        elif self.directive['objective_type'] == 'reach_combo': self.directive_progress = self.combo_multiplier
        # destroy_tiles_dash is handled where tiles are destroyed

        # Check for completion
        if self.directive_progress >= self.directive['value']:
            self.directive['completed'] = True
            self.game.sounds['directive_complete'].play()
            if self.directive['reward_type'] == 'coins': self.coins += self.directive['reward_value']
            elif self.directive['reward_type'] == 'item': self.current_item = self.directive['reward_value']
    ...
    def update_falling_tiles(self):
        ...
        for i, tile in sorted(enumerate(self.tile_drops), reverse=True):
            ...
            if self.player.dash_timer > 0 and r.colliderect(self.player.rect):
                ...
                if self.directive and not self.directive.get('completed') and self.directive['objective_type'] == 'destroy_tiles_dash': self.directive_progress += 1
            ...
            if check_pos_real in self.tiles:
                ...
                if place_pos[1] < 0: continue # Prevent placing tiles offscreen-top
            ...
            if tile_data['type'] == 'conduit' and 'static_shock' in self.active_curses:
                ...
                if player_on_tile:
                    ...
                    if tile_data['data']['timer'] > 120:
                        self.handle_death()
                        tile_data['data']['timer'] = -120 # Cooldown
                elif tile_data['data']['timer'] > 0:
                     tile_data['data']['timer'] = 0 # Reset if player leaves
                elif tile_data['data']['timer'] < 0:
                     tile_data['data']['timer'] += 1 * self.time_scale # Cooldown recovery
    ...
    def update_projectiles(self):
        for i, p in sorted(enumerate(self.projectiles), reverse=True):
            ...
            for k, data_node_rect in sorted(enumerate(self.data_nodes), reverse=True):
                if p.rect.colliderect(data_node_rect):
    ...
    def update_player(self):
        if not self.dead:
            ...
            if self.player.collisions['bottom'] and tile_pos_below_player in self.tiles:
                ...
                if tile_type == 'conveyor_l':
                    self.player.velocity[0] -= 0.15 * self.time_scale
                elif tile_type == 'conveyor_r':
                    self.player.velocity[0] += 0.15 * self.time_scale
    ...
    def use_item(self):
        ...
        # --- MODIFIED: Handle Module Recycler perk ---
    ...
    def render_plasma(self, surf):
        if self.plasma_y < self.game.DISPLAY_SIZE[1]:
            plasma_rect = pygame.Rect(0, int(self.plasma_y), self.game.DISPLAY_SIZE[0], self.game.DISPLAY_SIZE[1] - int(self.plasma_y))
            ...
            if self.master_clock % 4 == 0:
                pos = [self.random.randint(0, self.game.DISPLAY_SIZE[0]), self.plasma_y + 5]
    ...
    def render_borders(self, surf):
        ...
        border_to_blit = self.border_img.copy()
        light_border_to_blit = self.border_img_light.copy()
    ...
    def render_hud(self, surface):
        ...
        if self.directive:
            prop_y = self.game.DISPLAY_SIZE[1] - 32
            prop_text = self.directive['flavor_text']
            if self.directive.get('completed'):
                ...
                progress = f"({int(self.directive_progress)}/{self.directive['value']})"
