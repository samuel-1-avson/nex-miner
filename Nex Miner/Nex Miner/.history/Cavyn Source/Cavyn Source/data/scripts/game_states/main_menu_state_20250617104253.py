...
class MainMenuState(State):
    def __init__(self, game):
        ...
        # Dynamic Background
        try:
            self.bg_far = self.game.load_img(self.game.get_path('data', 'images', 'background.png'))
            self.bg_near = self.game.load_img(self.game.get_path('data', 'images', 'background_sunken.png'))
        ...
        self.platform_img = self.game.load_img(self.game.get_path('data', 'images', 'tile.png'))
    ...
    def render(self, surface):
        ...
        # Info Box (Bottom)
        info_rect = pygame.Rect(10, surface.get_height() - 22, surface.get_width() - 20, 18)
        render_panel_9slice(surface, info_rect, self.panel_img, 8)
        
        bank_txt = f"Bank: {self.game.save_data['banked_coins']}"
        bank_pos = (info_rect.left + 10, info_rect.top + 5)
        coin_icon = self.game.animation_manager.new('coin_idle').img
        if coin_icon: surface.blit(coin_icon, (bank_pos[0] - coin_icon.get_width() - 2, bank_pos[1] - 1))
        ...
        hs_pos = (info_rect.right - hs_w - 8, info_rect.top + 5)
    ...
    def render_menu_options(self, surface, options_list, current_selection_index, panel_rect):
        ...
        for i, option in enumerate(options_list):
            ...
            # Notification Logic
            notification_text = ""
            if option == "CHARACTERS" and 'characters' in self.game.unlock_notifications: notification_text = "!"
            if option == "PLAYER HUB" and 'artifacts' in self.game.unlock_notifications: notification_text = "!"
