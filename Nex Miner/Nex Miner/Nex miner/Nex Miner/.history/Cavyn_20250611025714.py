...
bounce_tile_img = load_img('data/images/bounce_tile.png') # <-- NEW
...
while True:
    ...
    if master_clock > 180:
        ...
        if game_timer > 10 + 25 * (20000 - min(20000, master_clock)) / 20000:
            ...
            # <-- MODIFIED: Add spawning logic for bounce blocks
    ...
    for tile_pos in tiles:
        ...
        if tile_type == 'bounce': # <-- NEW: Render bounce tile
    ...
    base_row = max(tiles, key=lambda x: x[1])[1] - 1 if tiles else WINDOW_TILE_SIZE[1] - 2 # <-- MODIFIED: Check if tiles exist
    filled = True
    for i in range(WINDOW_TILE_SIZE[0] - 2):
    ...
    if not dead:
        ...
        if collisions['bottom']:
            ...
            if tile_pos_below in tiles:
                ...
                # <-- MODIFIED: Check for both fragile and bounce tiles
                if tile_type_below == 'fragile':
                    if 'timer' not in tiles[tile_pos_below]['data']:
                        sounds['block_land'].play()
                        tiles[tile_pos_below]['data']['timer'] = 90
                elif tile_type_below == 'bounce': # <-- NEW: Bounce logic
                    player.velocity[1] = -9 # Powerful bounce
                    player.jumps = player.jumps_max # Reset jumps on bounce
                    sounds['super_jump'].play()
                    # Add a cool particle effect for bouncing
                    for i in range(40):
            ...
            if event.key in [K_e, K_x]:
                if not dead:
                    ...
                    if current_item == 'cube':
                        ...
                        stack_heights[place_pos[0] - 1] = place_pos[1]
                        # Check if base_row is calculated before using it here
