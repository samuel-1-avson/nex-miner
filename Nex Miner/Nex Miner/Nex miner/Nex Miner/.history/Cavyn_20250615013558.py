...
# This method is the most reliable way to ensure paths work regardless of
...
# --- Imports ---
# These are now imported after setting BASE_DIR to ensure they can also use it if needed.
...
class Game:
    ...
    def load_configs(self):
        ...
        try:
            # --- DEBUGGING AND VERIFICATION STEP ADDED ---
            config_paths = {
                ...
                'curses': self.get_path('data', 'configs', 'curses.json') # <-- ADDED
            }

            for name, path in config_paths.items():
                if not os.path.exists(path):
                    # This is a very specific error message that will solve the problem.
                    print(f"--- DEBUG INFO ---")
            ...
            # If all paths exist, proceed to open them.
            with open(config_paths['upgrades'], 'r') as f:
                self.upgrades = json.load(f)
            with open(config_paths['perks'], 'r') as f:
                self.perks = json.load(f)
            with open(config_paths['biomes'], 'r') as f:
                self.biomes = json.load(f)
            with open(config_paths['characters'], 'r') as f:
                self.characters = json.load(f)
            with open(config_paths['curses'], 'r') as f: # <-- ADDED
                self.curses = json.load(f)

        except (json.JSONDecodeError) as e:
            # This will catch errors if the file is found but is not valid JSON
        ...
        try:
            with open(SAVE_FILE, 'r') as f:
                ...
                if 'characters' not in data:
                    data['characters'] = {
                        "unlocked": ["cavyn"], # Default character is always unlocked
                        "selected": "cavyn"
                    }
    ...
    def run(self):
        ...
        while True:
            ...
            render_offset = [0, 0]
            if self.save_data['settings'].get('screen_shake', True): # <-- Corrected typo
