...
class GameplayState(State):
    ...
    def update_directive(self):
        if not self.directive or self.directive.get('completed', False):
            return

        # Update progress based on objective type using safe .get()
        objective_type = self.directive.get('objective_type')
        if objective_type == 'collect_coins':
            self.directive_progress = self.coins
        elif objective_type == 'reach_combo':
            self.directive_progress = self.combo_multiplier
        ...
        if self.directive_progress >= self.directive.get('value', float('inf')):
            ...
            if reward_type == 'coins' and reward_value:
                self.coins += reward_value
            elif reward_type == 'item' and reward_value:
                self.current_item = reward_value
    ...
    def update_falling_tiles(self):
        ...
        for i, tile in sorted(enumerate(self.tile_drops), reverse=True):
            ...
            if self.player.dash_timer > 0 and r.colliderect(self.player.rect):
                ...
                if self.directive and not self.directive.get('completed', False) and self.directive.get('objective_type') == 'destroy_tiles_dash':
                    self.directive_progress += 1
    ...
    def render_hud(self, surface):
        ...
        if self.directive:
            ...
            # --- FIX: Safe access for directive text ---
