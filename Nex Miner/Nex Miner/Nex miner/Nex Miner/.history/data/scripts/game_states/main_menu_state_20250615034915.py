...
class MainMenuState(State):
            ...
            if event.type == pygame.KEYDOWN:
                if self.in_submenu:
                    if event.key in [pygame.K_DOWN, pygame.K_s]:
                        self.submenu_index = (self.submenu_index + 1) % len(self.play_options)
                    elif event.key in [pygame.K_UP, pygame.K_w]:
                        self.submenu_index = (self.submenu_index - 1 + len(self.play_options)) % len(self.play_options)
    ...
    def select_submenu_option(self):
        ...
        if option == "CLASSIC":
            self.game.push_state(GameplayState(self.game, mode="classic"))
        elif option == "ZEN MODE":
            self.game.push_state(GameplayState(self.game, mode="zen"))
        elif option == "HARDCORE":
            self.game.push_state(GameplayState(self.game, mode="hardcore"))
        elif option == "CHALLENGES":
            # This would push a new ChallengeSelectState, which would then
            # push a GameplayState configured for the selected challenge.
            print("Note: Challenge Mode requires a ChallengeSelectState to be implemented.")
        self.in_submenu = False
    ...
    def render(self, surface):
        ...
        # Render main menu or submenu based on state
        if self.in_submenu:
    ...
    def render_menu_options(self, surface, options_list, current_selection_index):
        """Helper to render a list of menu options."""
        y_pos = 80
